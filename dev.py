...
from backend.core.services.dev_supervisor_control import (
    read_backend_restart_request,
...
BACKEND_RELOAD_MAX_WAIT_ENV = "CODEYUN_DEV_BACKEND_RELOAD_MAX_WAIT_SECONDS"
...
DEFAULT_BACKEND_RELOAD_MAX_WAIT_SECONDS = 600.0
DEFAULT_BACKEND_HOST = "0.0.0.0"
DEFAULT_BACKEND_PORT = 8000
DEFAULT_FRONTEND_PORT = 5173
BACKEND_HEALTH_PATH = "/api/health"
# Importing the application may need to recover/checkpoint a large SQLite WAL
# before uvicorn can bind the port.  Keep the supervisor from killing that
# legitimate startup work and creating an endless restart loop.
BACKEND_HEALTH_FAILURE_LIMIT = 60
...
def write_console_host_status(
    ...
    *,
    ...
    backend_health_failure_count=0,
    ...
    payload = {
        ...
        "healthy": (
            backend_healthy and frontend_healthy
            if backend_healthy is not None and frontend_healthy is not None
            else None
        ),
        "components": {
            "backend": {
                "healthy": backend_healthy,
                "consecutive_failures": backend_health_failure_count,
            },
            "frontend": {
                "healthy": frontend_healthy,
                "consecutive_failures": frontend_health_failure_count,
            },
        },
    }
...
class SupervisorConfig:
    ...
    backend_reload_max_wait_seconds: float
...
def parse_args(argv):
    ...
    parser.add_argument(
        "--restart-backend",
        action="store_true",
        help="Request an immediate backend restart from the running development supervisor.",
    )
...
def load_config(args):
    return SupervisorConfig(
        ...
        backend_reload_max_wait_seconds=read_env_float(
            BACKEND_RELOAD_MAX_WAIT_ENV,
            default=DEFAULT_BACKEND_RELOAD_MAX_WAIT_SECONDS,
            minimum=1.0,
        ),
    )
...
def start_backend(root_dir, env, python_executable, reload_mode, backend_host, backend_port):
    ...
    # A watchdog-started dev.py runs under pythonw.exe and has no console to
    # inherit.  Starting console-subsystem python.exe without CREATE_NO_WINDOW
    # would therefore create a new conhost window on every backend restart.
    # Standard handles still inherit normally, so visible dev.py sessions keep
    # their console output while watchdog sessions keep their redirected logs.
    return subprocess.Popen(cmd, cwd=root_dir, env=env, **no_window_subprocess_kwargs())
...
def probe_backend_http(port, path=BACKEND_HEALTH_PATH, timeout_seconds=2.0):
    request = urllib.request.Request(
        f"http://127.0.0.1:{port}{path}",
        headers={"Cache-Control": "no-cache"},
    )
    try:
        with urllib.request.urlopen(request, timeout=timeout_seconds) as response:
            ...
            return payload.get("status") == "ok" and payload.get("service") == "codeyun-backend"
...
def backend_reload_trigger(
    *,
    ...
    if force_requested:
        return "manual force request"
    if last_change_at is not None and now - last_change_at >= quiet_seconds:
        return f"{quiet_seconds:.1f}s quiet period"
    if first_change_at is not None and now - first_change_at >= max_wait_seconds:
        return f"{max_wait_seconds:.1f}s maximum wait"
    return None
...
def main():
    ...
    if args.restart_backend:
        request = request_backend_restart(source="cli", root_dir=root_dir)
        log(f"Backend restart requested: {request['request_id']}")
        return
    ...
    if config.backend_reload_mode == "outer":
        ...
        log(f"Backend reload maximum wait: {config.backend_reload_max_wait_seconds:.1f}s")
    ...
    backend_first_change_at = None
    backend_last_change_at = None
    backend_last_change_reason = None
    backend_exit_reported = False
    initial_restart_request = read_backend_restart_request(root_dir)
    last_restart_request_id = (
        str(initial_restart_request.get("request_id"))
        if initial_restart_request is not None
        and float(initial_restart_request.get("requested_at") or 0.0) < supervisor_started_at
        else None
    )
    backend_health_failure_count = 0
    backend_healthy = None
    ...
    try:
        ...
        while True:
            ...
            now = time.monotonic()
            if change_reason:
                if not backend_pending_change:
                    backend_first_change_at = now
            ...
            force_restart_requested = False
            restart_request = read_backend_restart_request(root_dir)
            if restart_request is not None:
                request_id = str(restart_request.get("request_id"))
                if request_id != last_restart_request_id:
                    last_restart_request_id = request_id
                    force_restart_requested = True
                    log(
                        "Manual backend restart requested "
                        f"({restart_request.get('source') or 'manual'}; {request_id})."
                    )

            backend_code = backend_proc.poll()
            reload_trigger = backend_reload_trigger(
                now=now,
                first_change_at=backend_first_change_at,
                last_change_at=backend_last_change_at,
                quiet_seconds=config.backend_reload_cooldown_seconds,
                max_wait_seconds=config.backend_reload_max_wait_seconds,
                force_requested=force_restart_requested,
            )
            if reload_trigger is not None:
                reason = backend_last_change_reason or "no pending file change"
                log(f"Backend reload triggered by {reload_trigger} after {reason}; restarting backend ...")
                ...
                backend_last_change_reason = None
                backend_exit_reported = False
            elif backend_code is not None:
                if backend_pending_change:
                    if not backend_exit_reported:
                        log(
                            f"Backend exited with code {backend_code} while files are changing; "
                            "waiting for the quiet period or maximum wait before restart."
                        )
                        backend_exit_reported = True
                else:
                ...
                backend_exit_reported = False
                backend_healthy = probe_backend_http(backend_port)
                if backend_healthy:
                    backend_health_failure_count = 0
                else:
                    backend_health_failure_count += 1
                    log(
                        f"Backend health probe failed ({backend_health_failure_count}/"
                        f"{BACKEND_HEALTH_FAILURE_LIMIT}): {BACKEND_HEALTH_PATH}"
                    )
                    if backend_health_failure_count == BACKEND_HEALTH_FAILURE_LIMIT and backend_pending_change:
                        log(
                            "Backend is unhealthy while files are changing; waiting for the quiet period "
                            "or maximum wait before restart."
                        )
                    elif backend_health_failure_count >= BACKEND_HEALTH_FAILURE_LIMIT and not backend_pending_change:
                        log("Backend process is alive but HTTP service is unhealthy. Restarting backend ...")
                        backend_proc = restart_backend(
                            root_dir,
                            env,
                            python_executable,
            ...
            if frontend_code is not None:
                ...
                frontend_healthy = probe_frontend_http(DEFAULT_FRONTEND_PORT, health_path)
                if frontend_healthy:
                    frontend_health_failure_count = 0
                else:
            ...
            if console_host_enabled:
                write_console_host_status(
                    ...
                    backend_health_failure_count=backend_health_failure_count,
                    frontend_healthy=frontend_healthy,
                    frontend_health_failure_count=frontend_health_failure_count,
                )
