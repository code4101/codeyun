...
*   **核心能力**:
    ...
    *   **考勤订单查询/退款入口**: 小鹅通、微信支付、考勤 PG、订单浏览器自动化的实际运行环境在 `codepc_mi15`。在 `codepc_mf` 或 AI 命令行里不要直接 import `kq5034.order_ops` / `backend.core.attendance.order` 去查单或查退款；正式入口是 CodeYun 高层 API：查订单/执行退款用 `POST /api/attendance/order-execute`，查退款明细用 `POST /api/attendance/order-refund-details`。这两个接口会读取考勤配置中的执行设备并转发到 `codepc_mi15` 的 `/api/device-control/attendance/order/...`。只有在实现或测试这些 API 本身时，才直接接触底层爬虫封装。处理课程配置、完成算法和 Step 1–6 前，还必须先读 [`考勤课程ABC分类约定`](./考勤课程ABC分类约定.md)：A 类是普通觉观/念住月课，B 类是禅宗/修道班，C 类是闯关，三类规则不得互套。
