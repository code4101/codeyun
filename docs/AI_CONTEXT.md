...
*   **核心能力**:
    ...
    *   **凡修逆向/图鉴上下文**: 凡人修仙传手游的资源、APK、IL2CPP、热更新 Lua、图鉴和抓包分析有独立 AI 交接文档：`docs/domains/fanxiu/context/FANXIU_REVERSE_CONTEXT.md`。后续 agent 接手相关任务时先读该文档，再看具体代码或外部导出目录。若用户提到增量更新凡修逆向、更新凡修图鉴或同步最新游戏数据，还必须遵守 `docs/domains/fanxiu/architecture/凡修逆向增量更新约定.md`：默认使用快照、diff、选择性合并的增量机制，不要无理由暴力全量重算或覆盖业务数据。若任务涉及凡修抓包服务、pcap、协议解析、储物袋、拜谒榜单、活动排行或运行态数据入库，先读 `docs/domains/fanxiu/architecture/凡修抓包服务架构约定.md` 和 `docs/domains/fanxiu/architecture/凡修抓包业务数据落库设计.md`：抓包监控器、解析队列、业务增量写入器和图鉴只读展示必须解耦，已探明业务域必须由抓包服务增量写入数据库；图鉴页绝对禁止触发抓包、解析、同步、重建或历史补扫。若用户对照图鉴页反馈解析/更新问题，优先处理抓包服务和业务写入链路，只有数据库已有正确事实但展示错误时才改图鉴页。若任务涉及凡修 GUI、data-annotation、Runtime、Scheduler、守护、MuMu 画面流或资产树，先读 `docs/domains/fanxiu/architecture/凡修GUI场景地图与图模型约定.md`、`docs/domains/fanxiu/architecture/凡修data-annotation命名约定.md` 和 `docs/domains/fanxiu/runbooks/凡修data-annotation运行设备约定.md`：`scene/frame` 是资产树场景帧节点，运行时输入称直播帧；Layer 0～2 使用统一识别图，Layer 3 只提供 unknown 的全图相似辅助信息，不能输出 scene id 或参与导航，资产目录和 children 不生成识别关系。当前凡修自动化只以 `codepc_mf` 为运行目标，`codepc_mi15` 旧凡修数据不再作为事实来源。若任务涉及凡修业务任务实现，先读 `docs/domains/fanxiu/architecture/凡修行为树业务能力约定.md`，优先复用场景移动、弹窗处理、滚动窗口安全查找点击和局部结构读取能力，不在单个任务里写死坐标、滚动次数或目标位置。若任务涉及补标、缺标注或新界面探索，先读 `docs/domains/fanxiu/guides/凡修data-annotation自动探索标注数据.md`。若任务涉及 `/fanxiu/data-annotation/runtime`、行为树、守护/作业清单、开关、tick 或运行日志，先读 `docs/domains/fanxiu/architecture/凡修行为树运行框架约定.md`。临时 Runtime 探针、自定义函数或 notebook 式排查优先提交 `runtime/cells/code` 代码 Cell，不要默认私有直调 `_DataAnnotationRuntimeRunner`；新动作代码优先用 `ctx.wait_scene(...)` / `ctx.go_scene(...)`，`ctx.wait_view(...)` / `runtime.goto_view(...)` 只作旧兼容。弹窗候选与业务 Layer 0 必须进入同一次图裁决，但只有同时具备场景身份和可执行中断动作的弹窗节点才可注入；弹窗动作完成后回到同一业务识别；不得在图裁决后按编号、分数或目录二次改判。同一节点若被当前步骤显式声明为业务预期，则由业务消费其动作；这只改变动作归属，不改变图返回的 scene id。
