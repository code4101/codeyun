...
Codex 日记导入的旧逻辑容易把“混合类别”误用成碎片聚合依据：只要多个消息共同命中若干星图类别，就可能被合成一个混合节点。更深一层的问题是它容易变成流水账式全面记录，只追求“发生过什么都写进去”，而不是捕捉当天真正重要的价值。这样会导致三个问题：

- 不相关任务被揉在一起，例如 PDF、DPI 截图、日记导入等只因为同属 CodeYun 语境而合并。
- 混合标签失去语义，变成“碎片桶”的解释，而不是表达一个原子任务确实跨多个性质。
- 大量节点落到“综合”，标题和正文都在覆盖细节，却没有回答核心做成了什么、突破了什么、沉淀了什么可复用成果。

新的目标是把“任务是否同一件事”和“最终星图类别是什么”解耦：

- 先按 Codex 原始会话/消息结构形成不可拆分任务段。
- 再从任务段中判断主价值，优先落到明确的工程、项目、业务或专项类别。
- 最后把同一天同一显式主类聚合为唯一节点，让同类事项形成一条完整日记。
- 混合类别只作为最终节点的次要权重，不参与任务段聚合。
- “综合”只作为无法识别主价值时的兜底，不应成为 CodeYun 日记的主要承载类。
...
同一天同一显式主类必须进入同一个最终节点；“日期 × 主分类”是活跃日记的唯一键。
分类修正会改变该键，因此手动分类、AI 分类和类别合并完成后都必须重新聚合，不能只改元数据而留下重复节点。

进度表达式由聚合后总时长生成；当天各分类再以最大分类工时作为共同分母。

节点标题不追求覆盖所有细节，应优先体现这一块最重要的核心工作、关键突破或主要事宜；零散操作放入正文条目并压缩成少量主线记录。标题只表达一件事，不用“与/和/及/、”把多个事项并列塞进一个标题。

工程、项目类价值信号要优先于泛 CodeYun 语境。例如“接口、页面、缓存、回归测试、权限、路由”等内容应落到编程/技术或工作/项目类，而不是因为 `project_label=CodeYun` 就进入 `CodeYun/综合`。
...
4. 增加测试覆盖：
   ...
   - 不同会话同日同显式主类聚合为一条。
   - 不同显式主类不能合并。
   - 分类修正后自动按新分类重聚合，且只保留一条活跃节点。
   - 混合次要类不改变主类。
   - 工程/项目主价值优先于 `CodeYun/综合` 和 `general`。
