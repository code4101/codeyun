...
```text
docs/automation/context/CodeYun自动化协作交接.md
```
...
- `handoff_id`: `docs/automation/context/CodeYun自动化协作交接.md` 中的条目编号
...
当 `docs/automation/context/CodeYun自动化协作交接.md` 中存在 `open` 条目时，代码健康优化应把它视为候选来源之一。
...
接手后更新 `docs/automation/context/CodeYun自动化协作交接.md`：
...
- 当前剩余文件范围：
  ...
  - `docs/automation/context/CodeYun代码健康长期优化上下文.md`
...
- `success_metric`: `README.md` 中的 `[docs/archive/自动部署恢复档案.md](../../)`、长期上下文中的 `%TEMP%\...docs_sync...`、`docs/automation/context/CodeYun代码健康长期优化上下文.md` 不再进入前排误报；真实缺失路径仍保留
- `risk`: low
...
- `object`: `docs/domains/fanxiu/context/FANXIU_REVERSE_CONTEXT.md` 旧 core 模块名漂移
...
- `object`: `docs/domains/fanxiu/context/FANXIU_REVERSE_CONTEXT.md` 前端页面路径漂移
...
- `success_metric`: `docs_sync_scan` 中 `docs/domains/fanxiu/context/FANXIU_REVERSE_CONTEXT.md` 的缺失路径候选清零；前端和 API 集成的说明性短语也改为非路径措辞
- `risk`: low
...
- `object`: `docs/automation/context/AI_CLUSTER_CONTEXT.md` 集群旧后端/前端路径漂移
...
- `object`: `docs/automation/context/AI_CLUSTER_CONTEXT.md` 集群旧路径和旧接口说明
...
- `success_metric`: 新路径均通过 `Test-Path`；旧路径/旧接口 `rg` 无剩余匹配；`docs_sync_scan` 中 `docs/automation/context/AI_CLUSTER_CONTEXT.md` 候选清零
- `risk`: low
...
- `object`: `docs/platform/conventions/standard与plugins结构约定.md` 模板路径误报
...
- `success_metric`: `docs/platform/conventions/standard与plugins结构约定.md` 的模板路径候选清零；真实缺失路径测试仍保留
- `risk`: low
...
- `object`: `docs/domains/fanxiu/architecture/凡修行为树运行框架约定.md` 旧 core 路径漂移
...
- `object`: `docs/domains/fanxiu/architecture/凡修逆向资源安全边界.md` 旧资源模块路径漂移
...
- `object`: `docs/domains/fanxiu/architecture/凡修逆向资源安全边界.md` 旧资源模块路径
...
- `object`: `docs/domains/notes/筛选与排序实现说明.md` 旧 notes/cluster 页面路径漂移
...
- `object`: `docs/platform/architecture/分层上下文压缩设计.md` 旧后端模块路径漂移
...
- `object`: `docs/platform/architecture/远端SubAgent能力设计.md` 旧 SubAgent 后端路径漂移
...
- `risk`: low
- `verification`: `rg -n "subagent|subagents|device_control_subagents" backend docs/platform/architecture/远端SubAgent能力设计.md`，一次只读 `docs_sync_scan`
...
- `object`: `docs/archive/自动部署恢复档案.md` 中的旧前端构建标记路径与异常引号路径
...
- `object`: `docs/archive/自动部署恢复档案.md` 中的历史前端构建标记路径与异常引号路径
