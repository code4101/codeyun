...
动态复查作业的“N 分钟后复查”必须以业务成功确认时刻为基准，不得使用本次 attempt 开始时刻。例如论道作业开始后经过场景移动、抓包和入座确认才成功，下次半小时复查应从确认入座时起算。若误用 attempt 开始时间，耗时较长时会让 `next_time` 过早；后续 immediate 失败又保留该已逾期时间，就会形成连续重跑。
...
弹窗守护把资产树“弹窗”目录下的场景递归打平为单次 Layer 0 候选域。只有具有显式场景身份的节点参与同帧并行识别；资产嵌套不产生识别边，无身份素材自动忽略。所有有效命中统一交给识别图选择最具体节点，守护只执行该节点已有的安全标注动作；不得在命中通用弹窗后再做第二层子场景识别。没有有效命中时守护返回未处理并放行下游任务。
