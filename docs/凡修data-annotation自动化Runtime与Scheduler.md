...
工程 Scheduler 提交的是标准作业事务。标准作业事务的入口规则是：

1. 启动时先进入作业自己的稳定锚点，通常是 `#34 世界`。
2. 从锚点开始执行完整业务流程。
3. 只在本次流程已经到达的上下文内处理分支和幂等，例如“刚从 #249 点入 #250 后发现 `#250[领取]` 不可见，视为今日已无可领免费项”。
4. 收尾回稳定锚点，写入 `success/skipped/error`、`next_time/retry_after` 等结果字段。

标准作业事务禁止在启动时从中间场景恢复。即使当前真实画面看起来像某个详情页、奖励页或弹窗，工程作业也不能把它当成“上次做到这里”继续执行；它只能先通过通用场景移动回锚点，再重新走完整流程。原因是中间场景识别强依赖上下文，无上下文全局识别会让业务代码膨胀成复杂恢复状态机。

AI 调试 cell 不受“必须一次提交完整作业”的限制。AI 可以在自己明确持有上下文时分段提交：先进入入口页，再单独验证某个按钮、详情页或返回链，确认后继续提交下一段。这种分段属于调试者显式掌控的临时上下文，不是工程 Scheduler 的恢复机制；稳定后沉淀为正式作业时，仍要回到“锚点 -> 完整事务 -> 收尾”的结构。
