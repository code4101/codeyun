...
同一场景可以有多个 `isSceneIdentity` 锚点，Runtime 场景分数按“全部场景标识都命中”理解：同一 view 内多个场景标识取最低分，任意一个低于阈值都不能判定为该场景。检测/调试页可以把每个条件都展示出来，用来评估是哪一个锚点没有通过。不要把同一场景的多个备选锚点都勾成场景标识；如果只是备选，请只保留当前稳定的一组必要锚点。
