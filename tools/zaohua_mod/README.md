...
基于 BepInEx 6 的《造化仙缘》非官方插件集。源码保存在 CodeYun，游戏原始 DLL 和存档文件均不直接修改。
...
- `天道试炼`：独立难度插件，在 `至宝` 页面提供单独配置 Tab；主角不受影响，适用于不同至宝。
...
5. 打开 `至宝`，点击左侧 `天道试炼`，可配置 `NPC属性倍率` 和 `速度倍率`；重新打开后保持当前存档的值。
