...
class ReviewActionCliError(Exception):
...
def _resolve_task_id(task_space: dict, task_id: str, task_title: str) -> str:
    if bool(task_id) == bool(task_title):
        raise ReviewActionCliError("invalid_target", "必须且只能提供 --task-id 或 --task-title。")
    if task_id:
        return task_id
    ...
    if not matches:
        raise ReviewActionCliError("task_title_not_found", f"未找到标题为「{title}」的任务。", task_title=title)
    if len(matches) > 1:
        candidates = "\n".join(
            f"- {task.get('id')} [{task.get('status')}] {task.get('title')}"
            for task in matches[:12]
        )
        raise ReviewActionCliError(
...
def main() -> None:
    _configure_stdio()
    parser = argparse.ArgumentParser(description="Apply an AI task-space task review action.")
    ...
    parser.add_argument(
        "--task-title",
        default="",
        help="Exact task title to review; fails if not unique.",
    )
    parser.add_argument(
        "--action",
        choices=["mark_done", "request_archive_review", "keep_unarchived", "archive"],
        required=True,
        help="Task review action.",
    )
    parser.add_argument(
    ...
    def _apply(current: dict) -> dict:
        ...
        previous_status = str(task.get("status") or "") if task else ""
        return apply_task_review_action(current, task_id, args.action)

    try:
        saved = mutate_task_space(path, _apply)
    except ReviewActionCliError as exc:
        _abort(args, exc.code, exc.message, **exc.extra)
    except KeyError as exc:
        _abort(args, "task_missing", f"任务不存在：{task_id}", task_id=task_id)
    except ValueError as exc:
        _abort(args, "review_action_rejected", str(exc), task_id=task_id, action=args.action)
    ...
    print(f"AI task review action {args.action}: {task['title']}")
