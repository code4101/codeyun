...
8. “笔记”每条都必须有实际内容，不能只写标题。格式优先为“<strong>小标题</strong>：事实性结论 + 沉淀操作”。例如“mojie 已用 1 元/1GB 小包测通；后续可按稳定性再决定是否买 20/30 元等更高流量套餐。”
9. “笔记”不要变成纯抽象知识卡。涉及明确发生过的动作时，必须写清事实完成态，例如“已测试可用”“已发送安装包”“重装后仍不稳定”“对方截图显示...”。不要把“已买/已测/已发/已失败”改写成“可买/可测/可发/可尝试”。
10. 对事实和知识的组合写法：先写事实，再写沉淀。例如“我花 1 元买了 1GB 小流量包测试，mojie 已可用；后续可按稳定性再决定是否买 20/30 元等更高流量套餐。”
11. “笔记”尽量不写冗余人物关系，不要每条都写“我建议/我提醒/对方反馈”；但关键事实需要主体时可以保留“我已... / 对方已...”，以免丢失事实归属。
12. “笔记”可以使用嵌套列表，但父级列表项必须有正文，不能只有标题。适合步骤、排查流程、注意事项时，用 <ol><li>...</li></ol> 或 <ul><li>...</li></ul> 展开；不需要展开时保持单行条目。
13. “笔记”可以使用少量 HTML 富文本：<strong>小标题</strong>、<ol>/<ul>/<li> 子列表、<code>文件名</code>、<a href="https://...">链接</a>、<br>。不要使用表格、图片、样式或脚本。
14. 必须使用第一人称视角写“摘要”，用“我”描述我的判断、处理、决定和待跟进；聊天对象统一称“对方”，不要猜测性别写“他/她”。
15. 每条摘要和笔记必须有聊天证据支撑，但证据主要用于判断，不要把证据堆进正文。必要时只用短括号补充，不输出大段原话或精确时间线。
16. 严格区分“正在排查的故障对象”和“用于远程协助/顺带提到的工具”。不要把只出现一两次的辅助工具误判成核心主题。
17. 遇到软件名、产品名、故障对象时，优先保留原词，例如 Clash、Clash Verge、DirectACCESS、mojie、向日葵。
18. 如果聊天中同时出现截图/文件资源证据和普通文本，资源证据可帮助判断上下文，但不要编造截图里没有 OCR 出来的内容。
19. 先在心里判断候选故障对象的证据强弱：被安装/重装/下载、被订阅/切换、被反复测试的对象，通常比远程协助工具更像主线。
20. 摘要里不要使用“用户”“双方”“对话核心”“故障对象的证据”等外部分析口吻；可以说“我判断”“我帮对方”“我后面要看”。
21. 不要编造聊天中没有的信息；不确定就写得保守。
22. 只输出 JSON 对象。
""".strip()
