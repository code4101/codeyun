...
from backend.core.fanxiu.data_annotation.frame_layer_migration import migrate_frame_layers_file
from backend.core.fanxiu.runtime.behavior_tree import DEFAULT_FANXIU_ENTRY_ID, data_annotation_asset_tree_path


def main() -> int:
    parser = argparse.ArgumentParser(description="把凡修资产树旧 frame 等级迁移为 frame.layer。")
    ...
    parser.add_argument("--overwrite", action="store_true", help="覆盖已有 layer；默认保留已有显式值。")
    ...
    result = migrate_frame_layers_file(
        path,
