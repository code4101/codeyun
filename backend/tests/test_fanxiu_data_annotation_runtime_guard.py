...
def test_runtime_goto_view_forwards_layer0_wait_seconds(tmp_path, monkeypatch):
    runner = create_fanxiu_runtime_runner()
    ctx = {"images": {}}
    runtime = runtime_runner_core.FanxiuRuntime(runner, ctx, asset_tree_path=tmp_path / "asset_tree.json", stop_event=threading.Event())
...
def test_open_daily_entry_keeps_scrolling_when_ocr_rows_move(monkeypatch):
    ...
    monkeypatch.setattr(runner, "_cached_ocr_fragments", fake_ocr)
    ...
    def fake_scroll(view, shape, *, direction=None):
...
def test_runtime_ocr_matches_wraps_predicate(monkeypatch):
    runner = create_fanxiu_runtime_runner()
    runtime = runtime_runner_core.FanxiuRuntime(runner, {"images": {}}, stop_event=threading.Event())
    monkeypatch.setattr(runner, "_cached_ocr_fragments", lambda _ctx, _frame: [{"text": "修仙传 游历 人界"}])
...
def test_unknown_promo_offer_page_recovery_uses_existing_return_shape(monkeypatch):
    runner = create_fanxiu_runtime_runner()
    image249 = _image("储物袋", "0249.png", [
        {"id": "back", "kind": "rect", "title": "返回", "x": 0.05, "y": 0.932, "w": 0.08, "h": 0.047},
    ])
    ctx = {"images": {249: image249}}
    clicked: list[tuple[int | None, float, float]] = []

    monkeypatch.setattr(
        runner,
        "_cached_ocr_fragments",
        lambda _ctx, _frame: [{"text": "至尊甄选 活动倒计时 限购次数 适度娱乐 理性消费"}],
    )
    monkeypatch.setattr(runner, "_save_action_trace", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(
        runner,
...
def test_world_promotion_banner_does_not_trigger_unknown_promo_return(monkeypatch):
    runner = create_fanxiu_runtime_runner()
    image249 = _image("储物袋", "0249.png", [
        {"id": "back", "kind": "rect", "title": "返回", "x": 0.05, "y": 0.932, "w": 0.08, "h": 0.047},
    ])
    ...
    monkeypatch.setattr(
        runner,
        "_cached_ocr_fragments",
        lambda _ctx, _frame: [{"text": "至尊甄选 储物袋 角色 装备 星海 功法书"}],
    )
    monkeypatch.setattr(
        runner,
        "_click_frame_point",
        lambda *_args, **_kwargs: pytest.fail("世界页宣传词不能触发固定坐标返回"),
    )

    assert runner._recover_unknown_promo_offer_page(ctx, "world-frame") is False


def test_scene_jump_to_world_stops_on_world_semantics_before_unknown_recovery(tmp_path, monkeypatch):
    runner = create_fanxiu_runtime_runner()
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: pytest.fail("世界语义应在图像身份和 unknown 恢复前终止跳转"),
    )
    monkeypatch.setattr(
        runner,
        "_recover_unknown_promo_offer_page",
        lambda *_args, **_kwargs: pytest.fail("到达世界后不得继续促销页恢复点击"),
    )
    ...
    assert result == 34
    assert any("停止后续恢复点击" in log["message"] for log in runner.status()["logs"])
...
def test_go_scene_to_world_rechecks_fresh_frame_before_clicking_false_scene(tmp_path, monkeypatch):
    ...
    frames = iter(["false-daily-frame", "fresh-world-frame"])

    monkeypatch.setattr(runner, "_screencap", lambda _ctx: next(frames))
    monkeypatch.setattr(
        runner,
        "_target_world_scene_ocr_confirmed",
        lambda _ctx, frame, _target: frame == "fresh-world-frame",
    )
    monkeypatch.setattr(runner, "_identify_scene_number", lambda _ctx, _frame: (69, 100.0))
    monkeypatch.setattr(
        runner,
    ...
    assert result == "success"
    assert any("取消原计划恢复点击" in log["message"] for log in runner.status()["logs"])
...
def test_auto_close_guard_images_flatten_popup_domain_and_require_scene_identity():
    runner = create_fanxiu_runtime_runner()
    top_level = _image("所有提示窗口", "0047.jpg", [{"id": "identity-47", "isSceneIdentity": True}])
    nested = _image("拍卖", "0028.jpg", [{"id": "identity-28", "sceneIdentityRole": "required"}])
    ...
    candidates = runner._auto_close_guard_images([
        ...
        {"type": "folder", "title": "登录弹窗", "children": [login]},
        {
            ...
            "children": [
                top_level,
                    {"type": "folder", "title": "所有提示窗口", "children": [nested, no_identity]},
            ],
        },
    ])
...
def test_identify_scene_number_does_not_return_layer3_without_identity(monkeypatch):
    runner = create_fanxiu_runtime_runner()
...
def test_navigation_scene_id_requires_parent_identity_when_frame_is_available(monkeypatch):
    runner = create_fanxiu_runtime_runner()
    parent = _image("世界", "0034.jpg", [
        {"id": "world-id", "kind": "rect", "title": "大地图", "sceneIdentityRole": "required"},
    ])
...
def test_go_scene_short_circuits_after_layer3_child_maps_to_target(monkeypatch, tmp_path):
    runner = create_fanxiu_runtime_runner()
    parent = _image("世界", "0034.jpg", [
        {"id": "world-id", "kind": "rect", "title": "大地图", "sceneIdentityRole": "required"},
    ])
    ...
    monkeypatch.setattr(runner, "_identify_scene_number", lambda _ctx, _frame: (68, 86.0))
    monkeypatch.setattr(runner, "_scene_score", lambda _ctx, image, _frame: 100.0 if image is parent else 86.0)

    result = runner._run_direct_runtime_action(
        lambda: runner._go_scene_task(ctx, tmp_path / "asset-tree.json", 34, threading.Event()),
        stop_event=threading.Event(),
        tick_seconds=0.01,
    )
...
def test_wait_scene_jump_treats_layer3_child_landing_as_source_stall(monkeypatch, tmp_path):
    runner = create_fanxiu_runtime_runner()
    ...
    result = runner._run_direct_runtime_action(
        lambda: runner._wait_scene_jump_result(
            ...
            source_scene_id=34,
            target_scene_id=69,
            edge={"source_id": 34, "image": parent, "shape": daily_shape, "target_ids": [69]},
            stop_event=threading.Event(),
...
def test_go_scene_replans_after_direct_entry_stalls_before_failing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runner, "_click_scene_route_shape", lambda _ctx, _image, shape, _frame: clicked.append(shape["title"]))
...
def test_go_scene_prefers_recognized_scene_shape_then_uses_default_recovery(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runner, "_target_world_scene_ocr_confirmed", lambda _ctx, frame, target: target == 34 and frame == "scene-34")
...
def test_auto_close_guard_tick_clicks_graph_selected_popup(tmp_path, monkeypatch):
    runner = create_fanxiu_runtime_runner()
    ...
    tree = [
        {
            ...
            "title": "弹窗",
            "children": [
                _image("所有提示窗口", "0047.jpg", [{"id": "identity-47", "isSceneIdentity": True}, blank_shape]),
                {
                    "type": "folder",
                    "title": "所有提示窗口",
                    "children": [_image("拍卖", "0028.jpg", [{"id": "identity-28", "isSceneIdentity": True}, nested_shape])],
                },
            ],
        },
    ]
    ...
    monkeypatch.setattr(runner, "_scene_score", lambda _ctx, image, _frame: 100 if image["title"] == "所有提示窗口" else 0)
...
def test_auto_close_guard_requires_high_confidence_during_task(tmp_path, monkeypatch):
    ...
    clicked: list[tuple[str, str]] = []
    monkeypatch.setattr(runner, "_scene_score", lambda _ctx, image, _frame: 85 if image["title"] == "所有提示窗口" else 0)
    monkeypatch.setattr(runner, "_click_shape", lambda _ctx, image, shape, _frame=None, **_kwargs: clicked.append((image["title"], shape["title"])))
...
def test_auto_close_guard_clicks_popup_blank_without_jump_target(tmp_path, monkeypatch):
        ...
        "type": "folder",
            ...
            "children": [
                _image("封魔杀", "0059.png", [
                    ...
                    {"id": "blank", "kind": "rect", "title": "空白", "x": 0.1, "y": 0.1, "w": 0.2, "h": 0.1},
                ]),
        ],
    }]
    path = tmp_path / "asset_tree.json"
    ...
    clicked: list[str] = []
    monkeypatch.setattr(runner, "_scene_score", lambda _ctx, image, _frame: 100 if image["title"] == "封魔杀" else 0)
...
def test_auto_close_guard_handles_disconnect_reconnect_before_generic_popup(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runner, "_cached_ocr_fragments", lambda _ctx, _frame: lines)
...
def test_auto_close_guard_specific_popup_84_clicks_no_more_prompt_only_when_unchecked(tmp_path, monkeypatch):
    runner = create_fanxiu_runtime_runner()
    ...
    specific_84 = _image("自动切磋", "0084.png", [
    ...
    tree = _popup_domain(popup_47, specific_84)
    path = tmp_path / "asset_tree.json"
    path.write_text(json.dumps(tree), encoding="utf-8")
    ...
    monkeypatch.setattr(runner, "_scene_score", lambda _ctx, image, _frame: 100 if image["title"] in {"所有提示窗口", "自动切磋"} else 0)
...
def test_auto_close_guard_specific_popup_84_checked_clicks_confirm(tmp_path, monkeypatch):
    runner = create_fanxiu_runtime_runner()
    no_more_prompt_shape = {"id": "no-more", "kind": "rect", "title": "不再提示", "x": 0.3, "y": 0.5, "w": 0.1, "h": 0.1}
    confirm_shape = {"id": "confirm", "kind": "rect", "title": "确认", "x": 0.4, "y": 0.6, "w": 0.2, "h": 0.1}
    specific_84 = _image("自动切磋", "0084.png", [
    ...
    path = tmp_path / "asset_tree.json"
    path.write_text(json.dumps(_popup_domain(popup_47, specific_84)), encoding="utf-8")
...
def test_auto_close_guard_popup_84_defers_when_confirm_actions_are_disabled(tmp_path, monkeypatch):
    runner = create_fanxiu_runtime_runner()
    no_more_prompt_shape = {"id": "no-more", "kind": "rect", "title": "不再提示", "x": 0.3, "y": 0.5, "w": 0.1, "h": 0.1}
    confirm_shape = {"id": "confirm", "kind": "rect", "title": "确认", "x": 0.4, "y": 0.6, "w": 0.2, "h": 0.1}
    specific_84 = _image("自动切磋", "0084.png", [
    ...
    assert not runner._auto_close_popup_guard_step(
        runner._fanxiu_runtime({"entry": object()}, path, "data:image/png;base64,frame"),
...
def test_auto_close_guard_specific_popup_86_clicks_confirm(tmp_path, monkeypatch):
    runner = create_fanxiu_runtime_runner()
    confirm_shape = {"id": "confirm", "kind": "rect", "title": "确认", "x": 0.4, "y": 0.6, "w": 0.2, "h": 0.1}
    specific_86 = _image("离开场景", "0086.png", [
        {"id": "identity", "kind": "rect", "title": "离开场景", "isSceneIdentity": True, "x": 0.2, "y": 0.4, "w": 0.5, "h": 0.1},
        confirm_shape,
    ])
    popup_47 = _image("所有提示窗口", "0047.jpg", [{"id": "blank", "kind": "rect", "title": "空白", "x": 0.1, "y": 0.8, "w": 0.2, "h": 0.1, "sceneJumpTarget": "-1"}])
    path = tmp_path / "asset_tree.json"
    path.write_text(json.dumps(_popup_domain(popup_47, specific_86)), encoding="utf-8")

    clicked: list[tuple[str, str]] = []
    monkeypatch.setattr(runner, "_popup_score", lambda _ctx, image, _frame: 70 if image["title"] in {"所有提示窗口", "离开场景"} else 0)
    monkeypatch.setattr(runner, "_scene_score", lambda _ctx, image, _frame: 100 if image["title"] in {"所有提示窗口", "离开场景"} else 0)
    monkeypatch.setattr(runner, "_click_shape", lambda _ctx, image, shape, _frame=None, **_kwargs: clicked.append((image["title"], shape["title"])))
...
def test_auto_close_guard_specific_popup_289_clicks_confirm(tmp_path, monkeypatch):
    runner = create_fanxiu_runtime_runner()
    confirm_shape = {"id": "confirm", "kind": "rect", "title": "确认", "x": 0.54, "y": 0.64, "w": 0.18, "h": 0.06}
    specific_289 = _image("0289.png", "0289.png", [
        {"id": "identity", "kind": "rect", "title": "退出灵脉", "isSceneIdentity": True, "x": 0.3, "y": 0.42, "w": 0.4, "h": 0.08},
        confirm_shape,
    ])
    popup_47 = _image("所有提示窗口", "0047.jpg", [{"id": "blank", "kind": "rect", "title": "空白", "x": 0.1, "y": 0.8, "w": 0.2, "h": 0.1, "sceneJumpTarget": "-1"}])
    path = tmp_path / "asset_tree.json"
    path.write_text(json.dumps(_popup_domain(popup_47, specific_289)), encoding="utf-8")

    clicked: list[tuple[str, str]] = []
    monkeypatch.setattr(runner, "_popup_score", lambda _ctx, image, _frame: 70 if image["title"] in {"所有提示窗口", "0289.png"} else 0)
    monkeypatch.setattr(runner, "_scene_score", lambda _ctx, image, _frame: 100 if image["title"] in {"所有提示窗口", "0289.png"} else 0)
    monkeypatch.setattr(runner, "_click_shape", lambda _ctx, image, shape, _frame=None, **_kwargs: clicked.append((image["title"], shape["title"])))
...
def test_auto_close_guard_does_not_use_ocr_as_a_second_popup_recognition_layer(tmp_path, monkeypatch):
    runner = create_fanxiu_runtime_runner()
    confirm_shape = {"id": "confirm", "kind": "rect", "title": "确认", "x": 0.54, "y": 0.64, "w": 0.18, "h": 0.06}
    specific_289 = _image("0289.png", "0289.png", [
        {"id": "identity", "kind": "rect", "title": "退出灵脉", "isSceneIdentity": True, "x": 0.3, "y": 0.42, "w": 0.4, "h": 0.08},
    ...
    monkeypatch.setattr(runner, "_scene_score", scene_score)
    monkeypatch.setattr(runner, "_cached_ocr_fragments", lambda _ctx, _frame: [{"text": "是否离开当前场景 取消 确认"}])
    monkeypatch.setattr(runner, "_click_shape", lambda _ctx, image, shape, _frame=None, **_kwargs: clicked.append((image["title"], shape["title"])))
...
def test_auto_close_guard_specific_popup_287_clicks_confirm(tmp_path, monkeypatch):
    runner = create_fanxiu_runtime_runner()
    confirm_shape = {"id": "confirm", "kind": "rect", "title": "确认", "x": 0.58, "y": 0.64, "w": 0.2, "h": 0.05}
    specific_287 = _image("0287.png", "0287.png", [
        {"id": "identity", "kind": "rect", "title": "前往灵脉", "isSceneIdentity": True, "x": 0.36, "y": 0.44, "w": 0.32, "h": 0.04},
        confirm_shape,
    ])
    popup_47 = _image("所有提示窗口", "0047.jpg", [{"id": "blank", "kind": "rect", "title": "空白", "x": 0.1, "y": 0.8, "w": 0.2, "h": 0.1, "sceneJumpTarget": "-1"}])
    path = tmp_path / "asset_tree.json"
    path.write_text(json.dumps(_popup_domain(popup_47, specific_287)), encoding="utf-8")

    clicked: list[tuple[str, str]] = []
    monkeypatch.setattr(runner, "_popup_score", lambda _ctx, image, _frame: 100 if image["title"] in {"所有提示窗口", "0287.png"} else 0)
    monkeypatch.setattr(runner, "_scene_score", lambda _ctx, image, _frame: 100 if image["title"] in {"所有提示窗口", "0287.png"} else 0)
    monkeypatch.setattr(runner, "_click_shape", lambda _ctx, image, shape, _frame=None, **_kwargs: clicked.append((image["title"], shape["title"])))

    assert runner._auto_close_popup_guard_step(runner._fanxiu_runtime({"entry": object()}, path, "data:image/png;base64,frame"))
    assert clicked == [("0287.png", "确认")]


def test_auto_close_guard_specific_popup_355_clicks_cancel(tmp_path, monkeypatch):
    runner = create_fanxiu_runtime_runner()
    cancel_shape = {"id": "cancel", "kind": "rect", "title": "取消", "x": 0.29, "y": 0.65, "w": 0.11, "h": 0.03}
    specific_355 = _image("0355.png", "0355.png", [
        {
            "id": "identity",
            "kind": "rect",
            "title": "激活镇魔玺",
    ...
    popup_47 = _image("所有提示窗口", "0047.jpg", [
        ...
        {"id": "blank", "kind": "rect", "title": "空白", "x": 0.1, "y": 0.8, "w": 0.2, "h": 0.1},
    ])
    path = tmp_path / "asset_tree.json"
    path.write_text(json.dumps(_popup_domain(popup_47, specific_355)), encoding="utf-8")

    clicked: list[tuple[str, str]] = []
    monkeypatch.setattr(
        runner,
        "_scene_score",
        lambda _ctx, image, _frame: 100 if image["title"] == "0355.png" else 95,
    )
    monkeypatch.setattr(
        runner,
        "_scene_match_edges_for_candidates",
        lambda _ctx, _ids: [{"s": 47, "x": 355, "matched": True}],
    )
    monkeypatch.setattr(
        runner,
        "_click_shape",
        lambda _ctx, image, shape, _frame=None, **_kwargs: clicked.append((image["title"], shape["title"])),
    )
    ...
    assert clicked == [("0355.png", "取消")]
    assert runner.status()["last_guard_event"]["action"] == "click:取消"


def test_auto_close_guard_specific_popup_skips_business_only_confirm(tmp_path, monkeypatch):
    runner = create_fanxiu_runtime_runner()
    business_confirm = {
        "id": "confirm",
        "kind": "rect",
    ...
    specific_210 = _image("同游传道确认提示", "0210.png", [
        {"id": "identity", "kind": "rect", "title": "同游传道确认标识", "isSceneIdentity": True, "x": 0.18, "y": 0.43, "w": 0.67, "h": 0.075},
        business_confirm,
    ])
    popup_47 = _image("所有提示窗口", "0047.jpg", [{"id": "blank", "kind": "rect", "title": "空白", "x": 0.1, "y": 0.8, "w": 0.2, "h": 0.1, "sceneJumpTarget": "-1"}])
    path = tmp_path / "asset_tree.json"
    path.write_text(json.dumps(_popup_domain(popup_47, specific_210)), encoding="utf-8")

    clicked: list[tuple[str, str]] = []
    monkeypatch.setattr(runner, "_popup_score", lambda _ctx, image, _frame: 100 if image["title"] in {"所有提示窗口", "同游传道确认提示"} else 0)
    monkeypatch.setattr(runner, "_scene_score", lambda _ctx, image, _frame: 100 if image["title"] in {"所有提示窗口", "同游传道确认提示"} else 0)
    monkeypatch.setattr(runner, "_click_shape", lambda _ctx, image, shape, _frame=None, **_kwargs: clicked.append((image["title"], shape["title"])))
...
def test_auto_close_guard_uses_graph_edge_to_choose_more_specific_popup(tmp_path, monkeypatch):
    runner = create_fanxiu_runtime_runner()
    ...
    popup_47 = _image("所有提示窗口", "0047.jpg", [
        {"id": "identity-47", "isSceneIdentity": True},
        {"id": "parent-blank", "kind": "rect", "title": "空白", "x": 0.1, "y": 0.8, "w": 0.2, "h": 0.1},
    ])
    path = tmp_path / "asset_tree.json"
    path.write_text(json.dumps(_popup_domain(popup_47, event_popup, leave_popup)), encoding="utf-8")
    ...
    monkeypatch.setattr(runner, "_scene_score", popup_score)
    monkeypatch.setattr(runner, "_scene_match_edges_for_candidates", lambda _ctx, _ids: [{"s": 47, "x": 32}])
    monkeypatch.setattr(runner, "_click_shape", lambda _ctx, image, shape, _frame=None, **_kwargs: clicked.append((image["title"], shape["title"])))
...
def test_auto_close_guard_flattens_physically_nested_popup_scene(tmp_path, monkeypatch):
    runner = create_fanxiu_runtime_runner()
    generic_blank = {"id": "generic-blank", "kind": "rect", "title": "空白", "x": 0.1, "y": 0.8, "w": 0.2, "h": 0.1}
    nested_blank = {"id": "nested-blank", "kind": "rect", "title": "空白", "x": 0.1, "y": 0.1, "w": 0.2, "h": 0.1}
    physically_nested = _image("仅物理嵌套", "0032.png", [{"id": "identity-32", "isSceneIdentity": True}, nested_blank])
    popup_47 = _image("所有提示窗口", "0047.jpg", [{"id": "identity-47", "isSceneIdentity": True}, generic_blank])
    popup_47["children"] = [physically_nested]
    tree = [{"type": "folder", "title": "弹窗", "children": [popup_47]}]
    path = tmp_path / "asset_tree.json"
    path.write_text(json.dumps(tree), encoding="utf-8")

    clicked: list[tuple[str, str]] = []
    monkeypatch.setattr(runner, "_scene_score", lambda _ctx, image, _frame: 95 if image["title"] == "仅物理嵌套" else 90)
    monkeypatch.setattr(runner, "_click_shape", lambda _ctx, image, shape, _frame=None, **_kwargs: clicked.append((image["title"], shape["title"])))

    assert runner._auto_close_popup_guard_step(runner._fanxiu_runtime({"entry": object()}, path, "data:image/png;base64,frame"))
    assert clicked == [("仅物理嵌套", "空白")]
...
def test_runtime_behavior_tree_popup_84_uses_separate_ticks_for_checkbox_and_confirm(tmp_path, monkeypatch):
    ...
    def scene_score(_ctx, image, _frame):
        if len(clicked) >= 2:
            return 0
        return 90 if image["title"] in {"所有提示窗口", "自动切磋"} else 0

    def shape_score(_ctx, _image, shape, _frame, **_kwargs):
        return 0 if shape["title"] == "不再提示" and not clicked else 90
...
def test_popup_guard_restarts_scan_each_tick_so_actual_popup_order_can_differ(tmp_path, monkeypatch):
    ...
    tree = [{
        ...
        "title": "弹窗",
        "children": [
            _image(title, f"{index:04d}.jpg", [{"id": f"identity-{index}", "isSceneIdentity": True}, shapes[title]])
            for index, title in enumerate(shapes, start=1)
        ],
    }]
    path = tmp_path / "asset_tree.json"
    path.write_text(json.dumps(tree), encoding="utf-8")
    ...
    def scene_score(_ctx, image, _frame):
        if len(clicked) >= len(actual_order):
            return 0
        return 90 if image["title"] == actual_order[len(clicked)] else 0
...
def test_popup_guard_parallel_scores_choose_best_graph_candidate_without_an_edge(tmp_path, monkeypatch):
    runner = create_fanxiu_runtime_runner()
    tree = [{
        "type": "folder",
        "title": "弹窗",
        "children": [
            _image("图1", "0001.jpg", [{"id": "i1", "isSceneIdentity": True}, {"id": "s1", "kind": "rect", "title": "空白", "x": 0, "y": 0, "w": 1, "h": 1, "sceneJumpTarget": "-1"}]),
            _image("图2", "0002.jpg", [{"id": "i2", "isSceneIdentity": True}, {"id": "s2", "kind": "rect", "title": "空白", "x": 0, "y": 0, "w": 1, "h": 1, "sceneJumpTarget": "-1"}]),
            _image("图3", "0003.jpg", [{"id": "i3", "isSceneIdentity": True}, {"id": "s3", "kind": "rect", "title": "空白", "x": 0, "y": 0, "w": 1, "h": 1, "sceneJumpTarget": "-1"}]),
        ],
    }]
    path = tmp_path / "asset_tree.json"
    path.write_text(json.dumps(tree), encoding="utf-8")
    ...
    def score(_ctx, image, _frame):
        return {"图1": 10, "图2": 85, "图3": 90}[image["title"]]

    monkeypatch.setattr(runner, "_scene_score", score)
    monkeypatch.setattr(
        runner,
        "_scene_reference_similarity",
        lambda _ctx, image, _frame: {"图2": 82.0, "图3": 94.0}.get(image["title"]),
    )
    monkeypatch.setattr(runner, "_click_shape", lambda _ctx, image, _shape, _frame=None, **_kwargs: clicked.append(image["title"]))
...
def test_runtime_scene_terms_are_documented():
    ...
    assert "scene 是主术语" in text
    assert "frame 是底层细节" in text
...
@pytest.mark.parametrize(
    ...
    [
        (279, [(279, "收益", 284), (284, "领取", 279), (279, "返回", 34)]),
        (284, [(284, "领取", 279), (279, "返回", 34)]),
    ],
)
def test_daily_dongtian_claim_uses_actual_entry_landing(monkeypatch, start_scene_id, expected_clicks):
    ...
    class FakeRuntime:
        ...
        def wait_click_then_view(self, scene_id, shape_title, target_scene_id, **_kwargs):
            self.clicks.append((scene_id, shape_title, target_scene_id))
            yield BehaviorTreeStatus.RUNNING
...
def test_daily_lingmai_summary_popup_clicks_ocr_confirm_before_returning_world(monkeypatch):
    ...
    assert result == "success"
    assert runtime.actions[0] == ("click_frame_point", (302, 457.0, 1069.0))
    assert runtime.actions[-1] == ("goto_view", 34)
...
def test_daily_lingmai_frame_306_clicks_ocr_confirm_before_returning_world(monkeypatch):
    ...
    assert result == "success"
    assert runtime.actions[0] == ("click_frame_point", (306, 453.0, 1069.0))
    assert runtime.actions[-1] == ("goto_view", 34)
...
def test_daily_lundao_empty_seat_confirms_go_to_dojo_popup_before_continuing():
    ...
    class FakeRuntime:
        def __init__(self):
            self.scenes = iter([(298, 100.0, "empty"), (329, 100.0, "confirm"), (303, 100.0, "dialog"), (53, 100.0, "leave"), (34, 100.0, "world")])
            self.actions = []
    ...
    assert runtime.actions[2] == (
        "wait_click_then_view",
        (303, "对话", [373, 52, 53, 329, 301, 302, 186]),
        {"settle_seconds": 1.5, "timeout": 20.0},
    )
    ...
    class FakeRuntime:
        ...
        def wait_scene(self, *scenes, **kwargs):
            ...
            if scenes == (375, 295, 52):
                return 375
            if scenes == (373, 52, 329, 301, 302, 303, 186):
                return 373
...
def test_daily_lundao_scene_303_routes_through_post_battle_dialogue_without_long_retry():
    ...
    assert runtime.actions == [
        ...
        (
            "wait_scene",
            (52, 53, 329, 301, 302, 303, 186),
            {"timeout": 30.0, "label": "论道_座位：战后对话结束后等待入座"},
        ),
...
def test_daily_yaowang_stops_on_purchase_modal(monkeypatch):
    ...
    monkeypatch.setattr(runner, "_ocr_frame", lambda _frame, **_kwargs: {"tokens": _ocr_tokens_from_fragments(purchase_lines)})
    clicked: list[tuple[float, float]] = []
    monkeypatch.setattr(runner, "_click_frame_point", lambda _ctx, _image, x, y: clicked.append((x, y)))
...
def test_mail_selective_claim_scroll_limit_returns_skipped_retry(tmp_path, monkeypatch):
    ...
    ctx = {"entry": object(), "asset_tree_path": tmp_path / "asset_tree.json", "images": {121: image121}}
    ...
    monkeypatch.setattr(runner, "_fanxiu_runtime", lambda *_args, **_kwargs: FakeRuntime())
    monkeypatch.setattr(runner, "_identify_scene_number", lambda *_args, **_kwargs: (None, 0.0))
    monkeypatch.setattr(runner, "_fanxiu_runtime_scene_text", lambda *_args, **_kwargs: (121, 100.0, "mail-list", ""))
    monkeypatch.setattr(runner, "_runtime_mail_rows_from_frame", lambda *_args, **_kwargs: [])
    monkeypatch.setattr(
        runner,
        "_record_scheduler_task_discovered_retry_after",
        lambda task_id, retry_after, **kwargs: retries.append((task_id, retry_after, kwargs["last_result"])),
    )
...
def test_daily_xianyuan_excludes_xianyuan_duel_and_clicks_real_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runner, "_screencap", lambda _ctx: "frame")
    monkeypatch.setattr(runner, "_ocr_fragments", lambda _frame: next(ocr_calls))
    monkeypatch.setattr(runner, "_click_frame_point", lambda _ctx, _image, x, y: clicked.append((x, y)))
...
def test_daily_mojie_raid_opens_daily_entry_by_mojie(tmp_path, monkeypatch):
    ...
    assert calls == [
        ...
        ("ocr_numbers_in_shapes", 322, ("队伍数",), {"padding": 12}),
...
def test_daily_mojie_raid_skips_full_team_capacity_until_next_schedule(tmp_path, monkeypatch):
    """The three-team quota is alliance-wide for one raid cycle, not per player."""
    runner = create_fanxiu_runtime_runner()
    path = tmp_path / "asset_tree.json"
    path.write_text("[]", encoding="utf-8")
    ctx = {"asset_tree_path": path, "asset_tree": [], "images": {}}
    ...
    class FakeRuntime:
        def current_scene(self, _preferred, *, update=False):
            return 322, 100.0, "team-dialog"

        def ocr_text(self, _frame):
            return "是否创建队伍 队伍数量：3/3"

        def ocr_numbers_in_shapes(self, scene_id, shape_titles, **kwargs):
            assert (scene_id, tuple(shape_titles), kwargs) == (322, ("队伍数",), {"padding": 12})
            return [3, 3], "3/3"

        def wait_click_then_view(self, *_args, **_kwargs):
            raise AssertionError("3/3 时不能继续点击 #322")

    monkeypatch.setattr(runner, "_fanxiu_runtime", lambda *_args, **_kwargs: FakeRuntime())
    monkeypatch.setattr(
        runner,
        "_scheduler_task_next_time_from_schedule",
        lambda *_args, **_kwargs: "2026-07-17 21:30:00",
    )
    monkeypatch.setattr(
        runner,
        "_record_scheduler_task_discovered_retry_after",
    ...
    assert recorded == [
        (
            ("legacy-daily-mojie-raid", "2026-07-17 21:30:00"),
            {"task_type": "daily_mojie_raid", "label": "日常_奇袭魔界", "last_result": "skipped"},
        )
    ]
...
def test_daily_xianyuan_dialogue_runs_challenge_flow(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runner, "_ocr_fragments", lambda frame: lines_by_frame[frame])
...
def test_daily_xianyuan_count_empty_records_before_returning_world(monkeypatch):
    ...
    monkeypatch.setattr(runner, "_ocr_fragments", lambda frame: [{"text": "今日可挑战次数：0/3"}] if frame == "count-empty" else [])
...
def test_daily_boss_reward_and_cd_parsers():
    ...
    assert runtime_runner_core._parse_daily_boss_cd_seconds("12:20后刷新") == 740
    assert runtime_runner_core._parse_daily_boss_cd_seconds("12小时后消失") is None
    assert runtime_runner_core._parse_daily_boss_cd_seconds("06:33:27后刷新") is None
    assert runtime_runner_core._parse_daily_boss_cd_seconds_from_six_digits("刷新时间 001220") == 740
    assert runtime_runner_core._parse_daily_boss_cd_seconds_from_six_digits("刷新时间 120020") is None
    assert runtime_runner_core._parse_daily_boss_hp_percent("首领 命20% 自动战斗中") == 20


def test_daily_boss_recheck_time_is_capped_at_half_hour(tmp_path, monkeypatch):
    ...
    next_time = runner._record_daily_boss_recheck_time(
        {"__scheduler_task_id": "daily-boss"},
        seconds=12 * 60 * 60,
    )

    assert next_time == "2026-06-11 11:30:00"
...
def test_daily_boss_detail_records_next_reset_when_reward_is_empty(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runner, "_screencap", lambda _ctx: "frame")
    monkeypatch.setattr(runner, "_ocr_fragments_in_shapes", lambda *_args, **_kwargs: [{"text": "神识注视剩余奖励次数：0/3"}])
...
def test_daily_boss_watched_list_cd_returns_skipped_retry_after(tmp_path, monkeypatch):
    ...
    image178 = _image("首领列表", "0178.png", [
        {"id": "list", "kind": "rect", "title": "首领列表", "x": 0.05, "y": 0.15, "w": 0.9, "h": 0.65},
        {
            ...
            "h": 0.1,
            "children": [
                {"id": "refresh", "kind": "rect", "title": "刷新时间", "x": 0.45, "y": 0.33, "w": 0.25, "h": 0.04},
                {"id": "watch", "kind": "rect", "title": "注视中", "x": 0.74, "y": 0.33, "w": 0.16, "h": 0.04, "ocrText": "注视", "floatingItemAnchor": True},
            ],
        },
    ])
    ctx = {
    ...
    monkeypatch.setattr(runner, "_screencap", lambda _ctx: "frame")
    monkeypatch.setattr(runner, "_ocr_frame", lambda *_args, **_kwargs: {
        "tokens": _ocr_tokens_from_fragments([
            {"text": "刷新时间 000009", "x": 420, "y": 528, "w": 210, "h": 32},
            {"text": "注视中", "x": 675, "y": 528, "w": 100, "h": 32},
        ])
    })
    monkeypatch.setattr(runner, "_daily_boss_reward_remaining_from_scene", lambda *_args, **_kwargs: 2)
    monkeypatch.setattr(runner, "_return_daily_boss_to_world", lambda *_args, **_kwargs: iter(()))
...
def test_daily_boss_daily_list_completed_does_not_mark_scheduler_success(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runtime_runner_core, "_data_annotation_world_facts_path", lambda: tmp_path / "world_facts.json")
    monkeypatch.setattr(runtime_runner_core, "_now", lambda: datetime(2026, 6, 11, 11, 0, 0))
    runtime_runner_core._write_data_annotation_world_facts({
        "discoveries": {
            "task": {
                "daily-boss": {
                    "discovered_retry_after": "2026-06-11 10:30:00",
                    "retry_after": "2026-06-11 10:30:00",
                    "last_result": "skipped",
                }
            }
        }
...
def test_daily_boss_done_rechecks_rewards_after_half_hour_even_when_list_cd_is_read(tmp_path, monkeypatch):
    ...
    def fake_ocr_fragments_in_shapes(_frame, image, names, **_kwargs):
...
def test_daily_boss_done_records_half_hour_retry_even_when_last_reward_was_challenged(tmp_path, monkeypatch):
    monkeypatch.setattr(fanxiu_api, "_data_annotation_world_facts_path", lambda: tmp_path / "world_facts.json")
    monkeypatch.setattr(runtime_runner_core, "_data_annotation_world_facts_path", lambda: tmp_path / "world_facts.json")
    monkeypatch.setattr(runtime_runner_core, "_now", lambda: datetime(2026, 6, 11, 11, 0, 0))
    runner = create_fanxiu_runtime_runner()
    ...
    def fake_ocr_fragments_in_shapes(_frame, _image, _names, **_kwargs):
        return [{"text": ""}]
...
def test_manual_daily_boss_success_does_not_mark_scheduler_success(tmp_path, monkeypatch):
    ...
    assert saved.get("next_time") is None
...
def test_xianfu_learn_skill_skips_when_price_is_not_free_without_cd(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runner, "_ocr_fragments_in_shapes", lambda *_args, **_kwargs: [{"text": "200领悟一次"}])
    monkeypatch.setattr(runner, "_click_shape", lambda _ctx, _image, shape, *_args, **_kwargs: clicked.append(shape["title"]))
    monkeypatch.setattr(runner, "_go_scene_task", lambda *_args, **_kwargs: "success")
    monkeypatch.setattr(runner, "_return_xianfu_learn_skill_to_world", lambda _runtime: iter(()))
...
def test_xianfu_learn_skill_existing_cd_is_not_marked_success(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runner, "_screencap", lambda _ctx: "frame")
    monkeypatch.setattr(runner, "_identify_scene_number", lambda _ctx, _frame, _preferred=None: (176, 100.0))
    monkeypatch.setattr(runner, "_ocr_fragments_in_shapes", lambda *_args, **_kwargs: [{"text": "23:59:17后可免费抽取"}])
    monkeypatch.setattr(runner, "_click_shape", lambda _ctx, _image, shape, *_args, **_kwargs: clicked.append(shape["title"]))
    monkeypatch.setattr(runner, "_go_scene_task", lambda *_args, **_kwargs: "success")
    monkeypatch.setattr(runner, "_return_xianfu_learn_skill_to_world", lambda _runtime: iter(()))
