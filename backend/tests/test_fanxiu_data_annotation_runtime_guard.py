...
def test_auto_close_guard_popup_47_child_289_clicks_confirm(tmp_path, monkeypatch):
    runner = create_fanxiu_runtime_runner()
...
def test_manual_success_clears_matching_scheduler_retry_without_scheduler_id(tmp_path, monkeypatch):
    ...
    fanxiu_api._write_data_annotation_scheduler_tasks([task])
    fanxiu_api._write_data_annotation_world_facts({
        "discoveries": {
            "task": {
                "legacy-daily-signup": {
                    "id": "legacy-daily-signup",
                    ...
                    "discovered_next_time": "2026-06-06 05:00:00",
                    "updated_at": datetime(2026, 6, 6, 10, 0, 0).timestamp(),
                }
            }
        }
    })
