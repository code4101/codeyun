...
def test_shape_ocr_cache_miss_is_decisive_without_repeating_crop_ocr(monkeypatch):
    ...
    ctx = {
        ...
        "_ocr_lines_cache": {
            "frame": "frame",
            "options_key": "{}",
            "lines": [{"text": "其它文字", "x": 100, "y": 170, "w": 80, "h": 30}],
        },
    }
    calls: list[dict] = []
...
def test_go_scene_stops_after_direct_entry_stalls_without_low_risk_recovery(tmp_path, monkeypatch):
    runner = create_fanxiu_runtime_runner()
    ...
    assert saved["action_shape"] is entry_shape
    assert "不执行泛化恢复动作" in saved["history"][0]
...
def test_daily_lingmai_summary_popup_clicks_ocr_confirm_before_returning_world(monkeypatch):
    ...
    monkeypatch.setattr(
        runner,
        "_cached_ocr_lines",
        lambda *_args, **_kwargs: [{"text": "确认", "x": 407.0, "y": 1042.0, "w": 100.0, "h": 54.0}],
    )
...
def test_daily_lingmai_frame_306_clicks_ocr_confirm_before_returning_world(monkeypatch):
    ...
    monkeypatch.setattr(
        runner,
        "_cached_ocr_lines",
        lambda *_args, **_kwargs: [{"text": "确定", "x": 398.0, "y": 1042.0, "w": 110.0, "h": 54.0}],
    )
