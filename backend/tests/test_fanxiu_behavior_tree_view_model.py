...
def test_shape_match_uses_shared_full_frame_ocr_spatial_index(monkeypatch):
    ...
    assert result["resolved_box"] == {"x": 423.0, "y": 1526.0, "w": 56.0, "h": 43.0}
    assert result["fixed_box"] == {"name": "邮件", "x": 449.1, "y": 1452.8, "w": 90.0, "h": 110.4}
...
def test_shape_match_accepts_ocr_contains_even_when_similarity_is_low(monkeypatch):
    ...
    assert result["matched"] is True
    assert result["ocr_text"] == "邮"
    assert result["resolved_box"] == {"x": 459.0, "y": 1527.0, "w": 28.5, "h": 30.0}
    assert result["fixed_box"] == {"name": "邮件", "x": 449.1, "y": 1452.8, "w": 90.0, "h": 110.4}


def test_legacy_mail_claim_check_task_type_maps_to_mail_selective_claim():
    runner = create_behavior_tree_runtime_runner()

    assert runner._canonical_runtime_task_type("mail_claim_check") == "mail_selective_claim"
