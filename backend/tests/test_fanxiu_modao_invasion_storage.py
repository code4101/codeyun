...
def test_region_character_list_roundtrip(tmp_path, monkeypatch) -> None:
    storage_path = tmp_path / "fanxiu_inventory.json"
    monkeypatch.setattr(fanxiu_inventory, "get_inventory_storage_path", lambda: storage_path)

    saved = fanxiu_inventory.save_region_character_list(
        {
            "characters": [
                {
                    "region_name": "天澜圣殿",
                ...
                },
                {
                    ...
                    "role": "道友甲",
                    "attackText": "12.3兆",
                    "recordedDate": "2026-04-26",
                },
            ]
        }
    )

    loaded = fanxiu_inventory.load_region_character_list()

    assert saved == loaded
    assert loaded["characters"][0]["region_name"] == "天澜圣殿"
    assert loaded["characters"][0]["server_name"] == "岁序更替"
    assert loaded["characters"][0]["guild_name"] == "三清道宗"
    assert loaded["characters"][0]["role_name"] == "玉清ღ清锋"
    assert loaded["characters"][0]["attack"] == "50.8兆"
    assert loaded["characters"][0]["recorded_date"] == "2026-04-27"
    assert loaded["characters"][1]["id"] == "character-2"
    assert loaded["characters"][1]["guild_name"] == "落云宗"
    assert loaded["characters"][1]["role_name"] == "道友甲"
    assert loaded["characters"][1]["recorded_date"] == "2026-04-26"
