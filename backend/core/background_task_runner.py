...
BACKGROUND_TASK_SPECS: tuple[BackgroundTaskSpec, ...] = (
    ...
    BackgroundTaskSpec(
        ...
        title="凡修天机阁抢答爬虫",
        category="凡修",
        description="复用 xlproject 的天机阁爬虫，在有奖竞答窗口前运行自动抢答流程；默认沿用旧 xlserver 的周二、周三、周四 17:59:50 调度。",
        schedule_label=f"每周二/三/四 {FANXIU_TIANJIGE_QUIZ_RUN_TIME}",
        retry_label="失败后 10 分钟重试",
        action=enqueue_fanxiu_tianjige_quiz,
        manual_warning="会使用 xlproject 的虚拟环境访问凡修天机阁接口并执行自动抢答；默认只允许在 mi15 运行。",
        default_visible=False,
    ),
)
