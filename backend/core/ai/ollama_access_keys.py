...
from backend.core.ai.chat_user_config import AiChatUserConfigError, _decrypt_secret, _encrypt_secret, _mask_secret
from backend.models import AppSetting


OLLAMA_ACCESS_KEYS_SETTING_KEY = "system:ai_chat:ollama_access_keys"
OLLAMA_ACCESS_KEY_ALPHABET = "ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789"
OLLAMA_ACCESS_KEY_LABEL_PREFIX = "访问 Key"


def _normalize_access_key_id(value: str | None) -> str:
    normalized = (value or "").strip().lower()
    if not normalized:
        raise AiChatUserConfigError("访问 Key 标识不能为空")
    return normalized
...
def _generate_access_key_label(items: dict[str, dict[str, Any]]) -> str:
    ...
    for item in items.values():
        ...
        if not label.startswith(f"{OLLAMA_ACCESS_KEY_LABEL_PREFIX} "):
            continue
        suffix = label[len(OLLAMA_ACCESS_KEY_LABEL_PREFIX) + 1:]
        if suffix.isdigit():
            max_index = max(max_index, int(suffix))
    return f"{OLLAMA_ACCESS_KEY_LABEL_PREFIX} {max_index + 1}"


def _generate_access_key_plaintext() -> str:
    chars = [secrets.choice(OLLAMA_ACCESS_KEY_ALPHABET) for _ in range(24)]
    groups = ["".join(chars[index:index + 4]) for index in range(0, len(chars), 4)]
    return f"oky-{'-'.join(groups)}"


def _load_ollama_access_key_map(session: Session) -> dict[str, dict[str, Any]]:
    row = session.get(AppSetting, OLLAMA_ACCESS_KEYS_SETTING_KEY)
    ...
    for raw_key_id, raw_item in raw_items.items():
        ...
        label = str(raw_item.get("label") or "").strip() or "未命名访问 Key"
...
def _save_ollama_access_key_map(session: Session, items: dict[str, dict[str, Any]]) -> None:
    row = session.get(AppSetting, OLLAMA_ACCESS_KEYS_SETTING_KEY)
    if not items:
        if row is not None:
            session.delete(row)
    ...
    now = time.time()
    if row is None:
        row = AppSetting(key=OLLAMA_ACCESS_KEYS_SETTING_KEY, value=payload, updated_at=now)
    else:
        row.value = payload
        row.updated_at = now
    session.add(row)
...
def _serialize_access_key_summary(key_id: str, item: dict[str, Any]) -> dict[str, Any]:
    return {
        "id": key_id,
        "label": str(item.get("label") or "").strip() or "未命名访问 Key",
...
def list_ollama_access_keys(session: Session) -> list[dict[str, Any]]:
    items = _load_ollama_access_key_map(session)
    return [
...
def create_ollama_access_key(
    ...
    *,
    ...
    now = time.time()
    key_id = f"oky-{uuid.uuid4().hex[:12]}"
    items[key_id] = {
    ...
    _save_ollama_access_key_map(session, items)
    return {
...
def reveal_ollama_access_key(session: Session, key_id: str | None) -> dict[str, Any]:
    normalized_key_id = _normalize_access_key_id(key_id)
    items = _load_ollama_access_key_map(session)
    item = items.get(normalized_key_id)
    if not item:
        raise AiChatUserConfigError("指定的 Ollama 访问 Key 不存在")
    return {
...
def delete_ollama_access_key(session: Session, key_id: str | None) -> None:
    normalized_key_id = _normalize_access_key_id(key_id)
    items = _load_ollama_access_key_map(session)
...
def has_ollama_access_keys(session: Session) -> bool:
    return bool(_load_ollama_access_key_map(session))


def ensure_ollama_access_key_allowed(session: Session, api_key: str | None) -> None:
    items = _load_ollama_access_key_map(session)
    if not items:
        raise AiChatUserConfigError("当前还没有配置可分发的 Ollama 访问 Key，请联系管理员先生成")

    normalized_api_key = (api_key or "").strip()
    if not normalized_api_key:
        raise AiChatUserConfigError("Ollama 需要访问 Key，请填写管理员分发的 CodeYun 访问 Key")
    ...
    raise AiChatUserConfigError("Ollama 访问 Key 无效，请检查后重试")
