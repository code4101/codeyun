...
def _watchdog_process_payload(proc: psutil.Process, cmdline_parts: list[str] | None = None) -> CodeYunWatchdogProcess:
...
def list_codeyun_watchdog_processes(*, full_scan: bool = True) -> list[dict[str, Any]]:
...
def get_codeyun_watchdog_status(*, full_scan: bool = False) -> dict[str, Any]:
...
def start_codeyun_watchdog(wait_seconds: float = 1.0) -> dict[str, Any]:
    status = get_codeyun_watchdog_status(full_scan=True)
