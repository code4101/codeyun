...
FANXIU_TCP_DECODE_SCHEMA_VERSION = 8
...
def _trim_value(value: Any, *, max_items: int = 8, preserve_item_types: set[str] | None = None) -> Any:
    ...
    preserve_item_types = preserve_item_types or {
        "MailVo",
        "RewardItem",
        "RewardResult",
        "CostResult",
        ...
        "ActivityRankCrossServerVO",
        "ActivityRankTeamVO",
        # Quest membership is authoritative server state. Trimming it can
        # splice the beginning/end of a task ladder and falsely present an
        # incomplete activity as complete.
        "QuestEntryVO",
    }
    if isinstance(value, dict):
        output: dict[str, Any] = {}
...
def _patch_fanxiu_schema_long_list(schema: LuaPacketSchemaIndex) -> LuaPacketSchemaIndex:
    ...
    # Older/minimal schema adapters only expose list decoding.  Keep the
    # long-list and I18n number patches usable for those adapters instead of
    # requiring the newer dynamic-bean hook to exist.
    original_read_bean = getattr(schema, "_read_bean", None)

    def read_dynamic_bean(
        self: LuaPacketSchemaIndex,
        reader: VarintBinaryReader,
        expected: str | None = None,
        depth: int = 0,
    ) -> Any:
        """Decode BaseMessage fields that carry a negative dynamic wire type."""

        start = reader.pos
        bean_id = reader.read_int()
        if bean_id < -1:
            return self._read_base_by_type(
                reader,
                bean_id,
                depth=depth + 1,
            )
        reader.pos = start
        if original_read_bean is None:
            raise AttributeError("schema does not expose _read_bean")
        return original_read_bean(
            reader,
            expected=expected,
            depth=depth,
        )

    def read_sparse_list(
        self: LuaPacketSchemaIndex,
        reader: VarintBinaryReader,
        *,
        declared_count: int,
        ...
        """Decode a CList whose declared slots exceed its serialized items."""
        ...
        if flag == 1:
            ...
            if type_id < 0 and type_id > -44:
                ...
                return {
                    "_count": item_count,
                    "_declared_count": declared_count,
                    "_type_id": type_id,
                    "_sparse": True,
                    "items": values,
                }
            ...
            return {
                ...
                "_declared_count": declared_count,
                "_type_id": type_id,
                "_type": info.name if info else None,
                "_sparse": True,
                "items": values,
            }
        for _ in range(item_count):
    ...
    def read_list_with_raw_long_fallback(
        self: LuaPacketSchemaIndex,
        reader: VarintBinaryReader,
        write_method: str | None = None,
        depth: int = 0,
    ) -> Any:
        # CList may write the number of logical slots first and the number of
        # serialized non-empty items second.  The ordinary format has a 0/1
        # shared-type flag immediately after the count; any other byte is the
        # first byte of that second varint count.  PartnerArenaJoinerVO.teams
        # is a live example: 5 declared slots, 2 serialized teams.
        start = reader.pos
        try:
            declared_count = reader.read_int()
            if declared_count > 0:
                count_pos = reader.pos
                flag = reader.read_byte()
                if flag not in {0, 1}:
                    reader.pos = count_pos
                    item_count = reader.read_int()
                    sparse_flag = reader.read_byte()
                    if 0 <= item_count <= declared_count and sparse_flag in {0, 1}:
                        return read_sparse_list(
                            self,
                            reader,
    ...
    if original_read_bean is not None:
        schema._read_bean = MethodType(read_dynamic_bean, schema)
    return schema
...
def decode_fanxiu_tcp_pcap(
    ...
    *,
    ...
    persist: bool = True,
    data_dir: str | Path | None = None,
    persist_decoded_records: bool = True,
    ...
    if persist:
        ...
        if persist_decoded_records:
            try:
                from backend.core.fanxiu.history_museum.packet_capture.decoded_store import persist_fanxiu_packet_decoded_result
            ...
            result["decoded_db_sync"] = {
                ...
                "skipped_duplicate": 0,
                "skipped": True,
                "reason": "caller_batches_decoded_record_persistence",
            }
...
def _sync_fanxiu_packet_runtime_insights_after_decode(
    ...
    *,
    ...
    try:
        from backend.core.fanxiu.history_museum.packet_capture.insights import sync_fanxiu_packet_business_for_decode_result
...
def _normalize_fanxiu_worldline_activity_item(item: dict[str, Any], *, export_root: str | Path | None = None) -> dict[str, Any] | None:
    ...
    server_ids_value = source.get("serverIds")
    server_ids = _server_ids(server_ids_value)
    if isinstance(server_ids_value, dict):
        server_count = int(server_ids_value.get("_count") or len(server_ids))
    else:
...
def _iter_fanxiu_tcp_entries(
    ...
    *,
    ...
    """Yield decoded packet entries one source at a time.

    A historical store can contain tens of thousands of decoded JSON files.
    Keeping every parsed frame, its display text, and its display segments in
    one list multiplies the source size and can exhaust the packet daemon.
    Consumers that aggregate history must stream this iterator instead.
    """
    ...
    for source in sources:
        ...
        for index, frame in enumerate(frames):
            ...
            if include_display:
                entry["display_text"] = _describe_fanxiu_tcp_business_entry(parsed, export_root=export_root)
                entry["display_segments"] = _describe_fanxiu_tcp_business_entry_segments(
                    parsed,
                    export_root=export_root,
                )
            yield entry
...
def _build_fanxiu_tcp_entries(data_dir: str, export_root: str | Path | None = None) -> list[dict[str, Any]]:
    """Build the legacy UI entry list.

    Historical reducers must use ``_iter_fanxiu_tcp_entries`` directly.
    """
    return list(_iter_fanxiu_tcp_entries(data_dir, export_root=export_root))
