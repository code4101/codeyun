...
from backend.core.fanxiu.history_museum.packet_capture.activity_sync import get_fanxiu_activity_rank_records, sync_fanxiu_activity_packets
from backend.core.fanxiu.catalog.item import load_fanxiu_item_runtime_index
from backend.core.fanxiu.history_museum.packet_capture.business_store_legacy import (
...
PACKET_INSIGHT_SCHEMA_VERSION = 20
...
ACTIVITY_PACKET_PROTOCOLS = {
    "SM_WorldLineActivitySync",
    "SM_ActivitySync",
    "SM_ActivityRankSync",
}
FANXIU_CONFIGURED_SELF_ROLE_ID = "24082878061086206"
FANXIU_CONFIGURED_SELF_NAME_KEYWORD = "羊驼"
...
def _extract_resource_state_updates(
    parsed: dict[str, Any],
    entry: dict[str, Any],
    item_index: dict[str, Any],
) -> list[dict[str, Any]]:
    """Extract absolute resource states carried by reward/cost packets.

    ``RewardResult.amount`` is the delta, while ``ResourceVo.amount/history/borrow``
    is the authoritative state after applying that delta.  Persisting the latter
    makes resource collection independent from any particular activity.
    """

    packet_name = str(entry.get("name") or parsed.get("_class") or "")
    field = "rewards" if packet_name == "SM_RewardResult" else "costs"
    rows: list[dict[str, Any]] = []
    for result in _items(parsed.get(field)):
        if not isinstance(result, dict) or _as_int(result.get("type")) != 1:
            continue
        content = result.get("content")
        if not isinstance(content, dict):
            continue
        resource_type = _as_int(content.get("type"))
        amount = _as_int(content.get("amount"))
        history = _as_int(content.get("history"))
        borrow = _as_int(content.get("borrow"))
        if resource_type is None or amount is None or history is None:
            continue
        payload = _resource_row(content, item_index=item_index)
        payload.update(
            {
                ...
                "history": history,
                "borrow": borrow or 0,
                "available": amount - (borrow or 0),
                "delta": _as_int(result.get("amount")),
                "reason": parsed.get("reason"),
                "captured_at": entry.get("captured_at") or entry.get("decoded_at") or "",
                "protocol": packet_name,
                "evidence": _packet_evidence(entry),
            }
        )
        rows.append(payload)
...
def _latest_self_profile_identity_from_snapshot(snapshot: dict[str, Any]) -> dict[str, Any] | None:
    ...
    own_rows = [
        ...
        if str(row.get("role_id_text") or row.get("role_id") or "") == FANXIU_CONFIGURED_SELF_ROLE_ID
        or FANXIU_CONFIGURED_SELF_NAME_KEYWORD in str(row.get("name") or "")
    ]
    ...
    try:
        ...
        with Session(engine) as session:
            rows = session.exec(
                ...
                .where(FanxiuPlayerProfileRecord.role_id_text == FANXIU_CONFIGURED_SELF_ROLE_ID)
...
def _valid_configured_self_profile_identity(row: dict[str, Any]) -> bool:
    ...
    role_id = str(row.get("role_id") or "").strip()
    name = str(row.get("name") or "").strip()
    return role_id == FANXIU_CONFIGURED_SELF_ROLE_ID or FANXIU_CONFIGURED_SELF_NAME_KEYWORD in name
...
def _runtime_business_record_rows(snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for resource in wallet.get("resources") or []:
        ...
        resource_type = _as_int(resource.get("type"))
        amount = _as_int(resource.get("amount"))
        history = _as_int(resource.get("history"))
        borrow = _as_int(resource.get("borrow")) or 0
        if resource_type is not None and amount is not None and history is not None:
            state_payload = {
                **payload,
                "resource_type": resource_type,
                "available": amount - borrow,
                "protocol": "SM_Wallet",
            }
            rows.append(
                _business_record_row(
                    "resource_state",
                    f"currency:{resource_type}",
                    state_payload,
                    entity_id=resource_type,
                    entity_name=resource.get("name") or f"资源类型 {resource_type}",
                    protocol="SM_Wallet",
                )
            )

    for payload in snapshot.get("resource_state_updates") or []:
        if not isinstance(payload, dict):
            continue
        resource_type = _as_int(payload.get("resource_type"))
        ...
        rows.append(
            _business_record_row(
                ...
                payload,
                entity_id=resource_type,
                entity_name=payload.get("name") or f"资源类型 {resource_type}",
...
def _decoded_entries_from_decode_result(result: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    captured_at = str(
        ...
        or _capture_time_from_pcap_name(pcap_name)
        or decoded_at
    )
...
def _business_snapshot_from_decoded_entries(
    ...
    *,
    ...
    resource_state_updates: list[dict[str, Any]] = []
    ...
    for entry in entries:
        ...
        if name in {"SM_RewardResult", "SM_CostResult"}:
            resource_state_updates.extend(
                _extract_resource_state_updates(parsed, entry, item_index)
            )
        if name == "SM_ShowOther":
            row = _extract_show_other_profile(parsed, entry, export_root=export_root)
            if row:
    ...
    return {
        ...
        "resource_state_updates": resource_state_updates,
...
def sync_fanxiu_packet_business_for_decode_result(
    ...
    *,
    ...
    if names.intersection(ACTIVITY_PACKET_PROTOCOLS):
        activity_sync = sync_fanxiu_activity_packets(
            data_dir=data_dir,
            export_root=export_root,
            force=False,
    ...
    empty_profile_sync = {"created": 0, "updated": 0, "skipped_invalid": 0, "skipped_duplicate": 0}
    try:
        from sqlmodel import Session
        from backend.db import engine
...
def _persist_runtime_business_records_to_database(snapshot: dict[str, Any]) -> dict[str, Any]:
    ...
    player_profile_rows = _runtime_player_profile_rows(snapshot)
    empty_player_profile_sync = {"created": 0, "updated": 0, "skipped_invalid": 0, "skipped_duplicate": 0}
...
def build_fanxiu_packet_runtime_insights(
    *,
    ...
    item_index = _load_item_index(export_root=export_root)
    entries = _iter_fanxiu_tcp_entries(
    ...
    *,
    ...
    if activity_names:
        activity_sync = sync_fanxiu_activity_packets(
            ...
            export_root=export_root,
            force=False,
            decoded_entries=_decoded_entries_from_decode_result(result),
        )
    if not names.intersection(PACKET_RUNTIME_INSIGHT_PROTOCOLS):
...
def decode_and_sync_fanxiu_runtime_capture(
    ...
    *,
    ...
    business_sync_results: list[dict[str, Any]] = []
    ...
    for stream in stream_ids:
        result = decode_fanxiu_tcp_pcap(
            ...
            persist=True,
            data_dir=data_dir,
            persist_decoded_records=False,
            sync_after_decode=False,
            prune_storage_after_decode=False,
        )
        if sync_business:
            business_result = sync_fanxiu_packet_business_for_decode_result(
                result,
                data_dir=data_dir,
                export_root=export_root,
            )
            if isinstance(business_result, dict):
                business_sync_results.append(business_result)
    ...
    return {
        ...
        "business_sync_results": business_sync_results,
