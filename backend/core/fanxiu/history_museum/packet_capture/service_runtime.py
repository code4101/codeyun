...
from backend.core.fanxiu.history_museum.packet_capture.insight_worker import fanxiu_packet_insight_worker
from backend.core.fanxiu.history_museum.packet_capture.tcp_flow import FANXIU_TCP_DECODE_SCHEMA_VERSION
...
FANXIU_PACKET_SERVICE_MODULE = "backend.core.fanxiu.history_museum.packet_capture.fanxiu_packet_daemon"
FANXIU_PACKET_SERVICE_TITLE = "凡修抓包"
FANXIU_PACKET_SERVICE_COMMAND_SCHEMA_VERSION = 1
FANXIU_PACKET_SERVICE_MAX_PRIVATE_MB_ENV = "FX_PACKET_SERVICE_MAX_PRIVATE_MB"
DEFAULT_FANXIU_PACKET_SERVICE_MAX_PRIVATE_MB = 2048.0
...
_PACKET_SERVICE_MAINTENANCE_DISPATCH_LOCK = threading.Lock()
_PACKET_SERVICE_MAINTENANCE_COMMAND_THREADS: dict[str, threading.Thread] = {}
...
def _safe_process_memory_mb(proc: Any) -> tuple[float | None, float | None]:
    try:
    ...
    return (
        round(float(rss) / (1024 * 1024), 1) if rss is not None else None,
        round(float(private) / (1024 * 1024), 1) if private is not None else None,
    )


def _packet_service_max_private_mb() -> float:
    try:
        return max(
            256.0,
            float(
                os.getenv(FANXIU_PACKET_SERVICE_MAX_PRIVATE_MB_ENV)
                or DEFAULT_FANXIU_PACKET_SERVICE_MAX_PRIVATE_MB
            ),
        )
    except (TypeError, ValueError):
        return DEFAULT_FANXIU_PACKET_SERVICE_MAX_PRIVATE_MB
...
def list_fanxiu_packet_service_processes() -> list[dict[str, Any]]:
    ...
    for proc in process_runtime.process_candidates_by_name(PYTHON_PROCESS_NAMES):
        ...
        if not _matches_fanxiu_packet_service_process(proc):
            continue
        rss_mb, private_mb = _safe_process_memory_mb(proc)
        items.append(
    ...
    children_by_parent: dict[int, list[FanxiuPacketServiceProcess]] = {}
    for item in items:
        if item.parent_pid in matched_pids:
            children_by_parent.setdefault(int(item.parent_pid), []).append(item)

    def process_tree(root: FanxiuPacketServiceProcess) -> list[FanxiuPacketServiceProcess]:
        rows = [root]
        for child in children_by_parent.get(root.pid, []):
            rows.extend(process_tree(child))
        return rows

    roots = [item for item in items if item.parent_pid not in matched_pids]
    rolled_up: list[FanxiuPacketServiceProcess] = []
    for root in roots:
        tree = process_tree(root)
        rolled_up.append(
            replace(
                root,
                rss_mb=round(sum(float(item.rss_mb or 0) for item in tree), 1),
                private_mb=round(sum(float(item.private_mb or 0) for item in tree), 1),
            )
        )
    rolled_up.sort(key=lambda item: (item.started_at or 0, item.pid))
    return [asdict(item) for item in rolled_up]
...
def request_fanxiu_packet_service_catch_up(*, reason: str = "api", wait_seconds: float = 30.0) -> dict[str, Any]:
    response = submit_fanxiu_packet_service_command(
        "packet_facts_catch_up",
        reason=reason,
        wait_seconds=wait_seconds,
    )
    command_result = response.get("result") if isinstance(response.get("result"), dict) else {}
    service_result = command_result.get("result") if isinstance(command_result.get("result"), dict) else {}
    facts_ready = bool(
        response.get("status") == "completed"
        and response.get("ok")
        and command_result.get("ok")
        and service_result.get("caught_up")
    )
    return {
        **response,
        "completed": response.get("status") == "completed",
        "facts_ready": facts_ready,
        "service_status": str(service_result.get("status") or response.get("status") or "pending"),
    }
...
def request_fanxiu_packet_service_capture_ready(
    *,
    ...
    capture_ready = bool(
        ...
        and response.get("ok")
        and command_result.get("ok")
        and service_result.get("capture_ready")
    )
    return {
        **response,
        "completed": response.get("status") == "completed",
...
def _capture_runtime_is_ready(status: dict[str, Any]) -> bool:
    """Project capture internals into the single readiness fact callers need."""

    if not (
        status.get("running")
        and status.get("tcpdump_ready")
        and str(status.get("current_pcap_path") or "").strip()
    ):
        return False
    mode = str(status.get("capture_mode") or "").strip()
    if mode == "local-stream":
        return bool(status.get("stream_writer_alive") and not status.get("stream_writer_error"))
    if mode == "remote-file":
        return bool(str(status.get("current_remote_pcap_path") or "").strip())
    return False
    ...
    try:
        if action == "ensure_capture_ready":
            ...
            while time.monotonic() <= deadline:
                ...
                if _capture_runtime_is_ready(status):
                    return {
                        "ok": True,
                        "status": "ready",
                        "capture_ready": True,
...
def _process_packet_service_command(path: Path) -> dict[str, Any] | None:
    ...
    created_at_epoch = command.get("created_at_epoch")
    try:
        command_age_seconds = (
            max(0.0, time.time() - float(created_at_epoch))
            if created_at_epoch not in (None, "")
            else 0.0
        )
    except (TypeError, ValueError):
        command_age_seconds = 0.0
    if command_age_seconds > timeout_seconds:
        payload = {
            "ok": False,
            "command_id": command_id,
            "action": action,
            ...
            "expired": True,
            "command_age_seconds": command_age_seconds,
            "timeout_seconds": timeout_seconds,
            "error": "抓包服务命令已过期，不在服务重启后重放",
            "result": {},
        }
...
def process_pending_fanxiu_packet_service_commands(*, limit: int = 5) -> list[dict[str, Any]]:
    """Dispatch foreground commands immediately and maintenance independently.

    Historical maintenance may take minutes.  Running it synchronously in the
    daemon inbox used to prevent later foreground catch-up commands from even
    being received.  A maintenance command now owns a background maintenance
    lane; the inbox remains available to urgent foreground requests.
    """

    def command_priority(path: Path) -> int:
        command = _read_json(path, {})
        return 1 if isinstance(command, dict) and command.get("action") == "maintenance" else 0

    def run_maintenance(path: Path, key: str) -> None:
        try:
            _process_packet_service_command(path)
        finally:
            with _PACKET_SERVICE_MAINTENANCE_DISPATCH_LOCK:
                _PACKET_SERVICE_MAINTENANCE_COMMAND_THREADS.pop(key, None)

    processed: list[dict[str, Any]] = []
    paths = sorted(_iter_pending_packet_service_commands(), key=command_priority)
    for path in paths[: max(0, int(limit))]:
        command = _read_json(path, {})
        if isinstance(command, dict) and command.get("action") == "maintenance":
            key = os.fspath(path.resolve(strict=False))
            with _PACKET_SERVICE_MAINTENANCE_DISPATCH_LOCK:
                active = _PACKET_SERVICE_MAINTENANCE_COMMAND_THREADS.get(key)
                if active and active.is_alive():
                    continue
                thread = threading.Thread(
                    target=run_maintenance,
                    args=(path, key),
                    name="fanxiu-packet-maintenance-command",
                    daemon=True,
                )
                _PACKET_SERVICE_MAINTENANCE_COMMAND_THREADS[key] = thread
                thread.start()
            processed.append(
                {
                    "ok": True,
                    "accepted": True,
                    "action": "maintenance",
                    "command_id": str(command.get("command_id") or path.stem),
                }
            )
            continue
...
def build_fanxiu_packet_service_health(status: dict[str, Any]) -> dict[str, Any]:
    ...
    max_private_mb = _packet_service_max_private_mb()
    process_private_mb = max(
        (
            float(item.get("private_mb") or 0)
            for item in (status.get("processes") or [])
            if isinstance(item, dict)
        ),
        default=0.0,
    )
    if process_private_mb > max_private_mb:
        issues.append("daemon_memory_exhausted")
...
def write_fanxiu_packet_service_state(extra: dict[str, Any] | None = None) -> dict[str, Any]:
    payload = {
        ...
        "decoder_schema_version": FANXIU_TCP_DECODE_SCHEMA_VERSION,
        "pid": os.getpid(),
...
def get_fanxiu_packet_service_status(*, include_health: bool = True) -> dict[str, Any]:
    ...
    status = {
        ...
        "decoder_schema_version": int(state_payload.get("decoder_schema_version") or 0),
        "expected_decoder_schema_version": FANXIU_TCP_DECODE_SCHEMA_VERSION,
...
def _packet_service_recovery_reason(status: dict[str, Any]) -> str:
    """Return a narrow self-heal reason for a live but wedged realtime worker."""

    health = status.get("health") if isinstance(status.get("health"), dict) else {}
    issues = {str(item) for item in (health.get("issues") or [])}
    recoverable = issues.intersection(
        {
            "daemon_memory_exhausted",
            "maintenance_result_stale",
            "realtime_worker_not_running",
            "worker_state_stale",
            "realtime_result_stale",
        }
    )
    if not recoverable:
        return ""
    if "daemon_memory_exhausted" in recoverable:
        return ",".join(sorted(recoverable))
    process_ages: list[float] = []
    for item in status.get("processes") or []:
        ...
        try:
            ...
            continue
        if started_at > 0:
            process_ages.append(max(0.0, time.time() - started_at))
    # A newly launched daemon briefly exposes the previous state snapshot.
    # Do not turn that normal startup window into a restart loop.
    if process_ages and max(process_ages) < 180.0:
        return ""
    return ",".join(sorted(recoverable))
...
def start_fanxiu_packet_service(wait_seconds: float = 3.0) -> dict[str, Any]:
    ...
    if status.get("running"):
        if int(status.get("decoder_schema_version") or 0) == FANXIU_TCP_DECODE_SCHEMA_VERSION:
            recovery_reason = _packet_service_recovery_reason(status)
    ...
    while time.monotonic() <= deadline:
        ...
        if status.get("running"):
            ...
            return {
                "status": "started",
                "service": status,
                **({"recovered_from": recovery_reason} if recovery_reason else {}),
            }
