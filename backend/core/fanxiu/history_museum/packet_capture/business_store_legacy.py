...
def _packet_id_from_row(row: dict[str, Any]) -> str:
...
def upsert_fanxiu_packet_business_records(session: Session, rows: list[dict[str, Any]]) -> dict[str, int]:
    created = 0
    updated = 0
    skipped_invalid = 0
    skipped_duplicate = 0
    for row in rows:
        ...
        packet_id = _text(row.get("packet_id") or _packet_id_from_row(row) or _packet_id_from_row(payload))
        ...
        if existing:
            # Business records are current-state projections. Replaying an old
            # capture must never roll a newer absolute game fact backwards.
