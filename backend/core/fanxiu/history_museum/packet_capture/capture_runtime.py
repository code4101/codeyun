...
from backend.core.fanxiu.history_museum.packet_capture.android_proxy import DEFAULT_ADB_CANDIDATES
from backend.core.fanxiu.history_museum.packet_capture.tcp_flow import DEFAULT_FANXIU_SERVER_HOST, resolve_fanxiu_tcp_live_capture_dir
...
class FanxiuCaptureRuntimeService:
    ...
    def _adb_device_candidates(self) -> list[str]:
        ...
        try:
            devices_output = self._run_adb(["devices"], timeout=8)
            connected = self._parse_adb_devices(devices_output)
        except Exception as exc:
            self._log(f"adb devices scan failed: {exc}")
        # Dynamically prefer the connected instance that actually owns the game
        # process.  A remembered/default serial is only a fallback, never the
        # identity of the running account.
        if not configured_device_id and len(connected) > 1:
            candidates.extend(
                device_id
                for device_id in connected
                if self._game_pid_on_device(device_id) is not None
            )
        if self.device_id != DEFAULT_FANXIU_DEVICE_ID:
    ...
    def _game_pid_on_device(self, device_id: str) -> int | None:
        try:
            output = self._run_adb(
                ["-s", device_id, "shell", "pidof", self.package_name],
                timeout=6,
            )
        except Exception:
            return None
    ...
    def _detect_game_running(self) -> bool:
        pid = self._game_pid_on_device(self.device_id)
        with self._lock:
            self._game_pid = pid
    ...
    def _start_local_stream_tcpdump_locked(self, local_path: Path) -> None:
        ...
        # `adb shell -T` is still a shell transport.  On some Windows/ADB
        # combinations it inserts bytes between binary writes, producing a
        # pcap whose global header is valid but whose second record is corrupt.
        # `exec-out` is the ADB primitive intended for an unmodified binary
        # stdout stream (the same distinction used for screencap -p).
        command = [str(self._adb_path()), "-s", self.device_id, "exec-out", "sh", "-c", shell_command]
    ...
    def _cleanup_stale_local_tcpdump_adb_locked(self) -> None:
        ...
        for proc in psutil.process_iter(["pid", "name", "cmdline"]):
            ...
            if self.device_id not in command_line:
                continue
            if (
                "shell tcpdump" not in command_line
                and "shell -T tcpdump" not in command_line
                and "exec-out sh -c tcpdump" not in command_line
            ):
                continue
            if (
