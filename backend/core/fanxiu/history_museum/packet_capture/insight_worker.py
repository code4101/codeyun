...
DEFAULT_MAINTENANCE_INTERVAL_SECONDS = 6 * 60 * 60.0
DEFAULT_MAINTENANCE_PRESSURE_RETRY_SECONDS = 30 * 60.0
DEFAULT_MAINTENANCE_MIN_USER_IDLE_SECONDS = 120.0
DEFAULT_MAINTENANCE_MAX_CPU_PERCENT = 45.0
DEFAULT_STABLE_SECONDS = 8.0
DEFAULT_FAILED_RETRY_SECONDS = 600.0
DEFAULT_LIVE_CAPTURE_MAX_AGE_SECONDS = 24 * 60 * 60
DEFAULT_MAINTENANCE_DECODE_LIMIT = 2
DEFAULT_MAINTENANCE_BUSINESS_LATEST_LIMIT = 2
DEFAULT_MAINTENANCE_BUSINESS_HISTORICAL_LIMIT = 2
DEFAULT_MAINTENANCE_MAIL_LATEST_LIMIT = 4
DEFAULT_MAINTENANCE_MAIL_HISTORICAL_LIMIT = 4
DEFAULT_DECODE_MAX_STREAMS = 4
DEFAULT_LIVE_CAPTURE_SCAN_MULTIPLIER = 40
DEFAULT_DECODE_TIMEOUT_SECONDS = 30.0
DEFAULT_REALTIME_DECODE_TIMEOUT_SECONDS = 10.0
...
PACKET_BUSINESS_RULE_VERSION = 2
...
_CAPTURE_DECODE_LOCKS_GUARD = threading.Lock()
_CAPTURE_DECODE_LOCKS: weakref.WeakValueDictionary[str, threading.Lock] = weakref.WeakValueDictionary()
...
def _windows_user_idle_seconds() -> float | None:
    if os.name != "nt":
        return None
    try:
        import ctypes
...
def _scheduled_maintenance_resource_gate() -> dict[str, Any]:
    """Let historical repair run only while the host is genuinely idle."""

    idle_seconds = _windows_user_idle_seconds()
    if idle_seconds is not None and idle_seconds < DEFAULT_MAINTENANCE_MIN_USER_IDLE_SECONDS:
        return {
            "defer": True,
            "reason": "user_active",
            "user_idle_seconds": round(idle_seconds, 3),
        }
    try:
        cpu_percent = float(psutil.cpu_percent(interval=0.1))
    except (psutil.Error, OSError, ValueError):
        cpu_percent = 0.0
    if cpu_percent >= DEFAULT_MAINTENANCE_MAX_CPU_PERCENT:
        return {
            "defer": True,
            "reason": "host_cpu_busy",
...
def _validate_sealed_pcap(path: Path) -> tuple[bool, str]:
    """Reject structurally corrupt classic pcaps before claiming catch-up.

    tcpdump writes classic pcap here.  Checking only the magic header is not
    sufficient: a damaged ADB binary stream can keep that header intact while
    corrupting the next record boundary.
    """
    ...
    try:
        ...
        with path.open("rb") as file:
            header = file.read(24)
            if len(header) != 24:
                return False, "truncated_global_header"
            magic = header[:4]
            if magic in {b"\xd4\xc3\xb2\xa1", b"\x4d\x3c\xb2\xa1"}:
                ...
                return False, "unsupported_pcap_magic"
            offset = 24
            packet_count = 0
            while offset < size:
                record_header = file.read(16)
                if len(record_header) != 16:
                    return False, f"truncated_record_header_at_{offset}"
                _seconds, _fraction, captured_length, original_length = struct.unpack(
                    f"{endian}IIII", record_header
                )
                if captured_length > 262_144 or original_length > 262_144:
                    return False, f"invalid_record_length_at_{offset}"
                if captured_length > original_length:
                    return False, f"captured_length_exceeds_original_at_{offset}"
                payload = file.read(captured_length)
                if len(payload) != captured_length:
                    return False, f"truncated_record_payload_at_{offset}"
                offset += 16 + captured_length
                packet_count += 1
            if packet_count == 0:
                return False, "no_packets"
    except OSError as exc:
        return False, f"pcap_read_error:{exc}"
    return True, "valid"


def _capture_decode_lock(capture_sha256: str) -> threading.Lock:
    """Serialize writes for one deterministic decoded artifact, not the lanes."""

    with _CAPTURE_DECODE_LOCKS_GUARD:
        lock = _CAPTURE_DECODE_LOCKS.get(capture_sha256)
        if lock is None:
            lock = threading.Lock()
            _CAPTURE_DECODE_LOCKS[capture_sha256] = lock
        return lock
...
def _ensure_capture_runtime_from_packet_worker(*, data_dir: str | Path | None = None) -> dict[str, Any]:
    from backend.core.fanxiu.history_museum.packet_capture.capture_runtime import fanxiu_capture_runtime_service
...
def _decoded_capture_index(
    data_dir: str | Path | None = None,
) -> tuple[set[str], dict[str, set[int]], dict[str, list[dict[str, Any]]]]:
    """Build all decoded-capture indexes in one metadata pass."""
    ...
    if not root.is_dir():
        return digests, streams_by_digest, sources_by_digest
    for meta_path in root.glob("*/meta.json"):
        meta = _load_json(meta_path, {})
        if not isinstance(meta, dict) or not _decoded_meta_uses_current_decoder(meta):
            continue
...
def _iter_stable_live_pcaps(
    *,
    ...
    if candidate_limit is not None and len(candidates) > max(1, int(candidate_limit)):
        # Maintenance may face a six-figure live-capture directory.  Select a
        # bounded newest/oldest window from filename timestamps before stat().
        # This preserves fresh-data latency and historical progress without
        # touching every file's metadata in each cycle.
        ordered = sorted(candidates, key=lambda item: item.name)
        bounded = max(1, int(candidate_limit))
        oldest_count = bounded // 2
        newest_count = bounded - oldest_count
        candidates = ordered[:oldest_count] + ordered[-newest_count:]
    rows: list[Path] = []
...
def _capture_has_current_decoded_record(
    ...
    *,
    ...
    return bool(_current_decoded_sources_for_capture(path, data_dir=data_dir))


def _current_decoded_sources_for_capture(
    path: Path,
    *,
    data_dir: str | Path | None = None,
    digest: str = "",
) -> list[dict[str, Any]]:
    """Read only one capture's deterministic decoded records."""
    try:
        capture_digest = digest or _sha256_file(path)
    except OSError:
        return []
    prefix = f"{path.stem}_{capture_digest[:12]}_stream"
    sources: list[dict[str, Any]] = []
    for meta_path in resolve_fanxiu_tcp_store_root(data_dir).glob(f"{prefix}*/meta.json"):
        meta = _load_json(meta_path, {})
        if str(meta.get("capture_sha256") or "") != capture_digest:
            continue
            ...
            sources.append(
                {
                    ...
                    "created_at": meta.get("created_at") or "",
                    "source_kind": "record",
                    "source_pcap": meta.get("source_pcap") or str(path),
                    "stored_pcap": meta.get("stored_pcap") or "",
                    "stream": int(meta.get("stream") or 0),
...
def _sync_business_after_decoded(
    ...
    *,
    ...
    mail_sync = {
        "ok": True,
        "skipped": True,
        "reason": "mail_runtime_memory_is_authoritative",
...
def _sync_historical_business_backlog(
    *,
    ...
    decoded_business_sync = sync_fanxiu_decoded_business_backlog(
        data_dir=data_dir,
        latest_limit=DEFAULT_MAINTENANCE_BUSINESS_LATEST_LIMIT,
        historical_limit=DEFAULT_MAINTENANCE_BUSINESS_HISTORICAL_LIMIT,
    )
    # Scheduled maintenance writes bounded decoded batches through the same
    # idempotent database reducers as realtime ingestion.  The legacy JSON
    # compatibility snapshots require a full history replay and must never be
    # rebuilt inside the long-lived packet daemon.
    packet_runtime_sync = {
        "ok": True,
        "skipped": True,
...
def sync_fanxiu_decoded_business_backlog(
    *,
    ...
    state, rule_version_reset = _business_backlog_state_for_current_rule(loaded_state)
    rule_is_current = bool(
        isinstance(loaded_state, dict)
        and int(loaded_state.get("business_rule_version") or 0) == PACKET_BUSINESS_RULE_VERSION
    )
    if rule_is_current and bool(loaded_state.get("historical_replay_complete", True)):
        return {
            "schema_version": PACKET_INSIGHT_WORKER_SCHEMA_VERSION,
            "business_rule_version": PACKET_BUSINESS_RULE_VERSION,
            "ok": True,
            "skipped": True,
            "reason": "historical_replay_complete",
            "source_count": int(loaded_state.get("source_count") or 0),
            "selected_count": 0,
            "processed_count": 0,
            "changed": False,
    ...
    historical_replay_complete = bool(
        int(historical_limit) > 0
        and historical_sources
        and sources
        and _decoded_source_key(historical_sources[-1]) == _decoded_source_key(sources[0])
    )
    payload = {
        "schema_version": PACKET_INSIGHT_WORKER_SCHEMA_VERSION,
        "business_rule_version": PACKET_BUSINESS_RULE_VERSION,
        ...
        "historical_replay_complete": historical_replay_complete,
    }
...
def sync_fanxiu_mail_business_backlog(
    *,
    ...
    """Legacy packet-mail backlog is permanently outside the active pipeline."""
    return {
        "schema_version": PACKET_INSIGHT_WORKER_SCHEMA_VERSION,
        "business_rule_version": PACKET_BUSINESS_RULE_VERSION,
        "ok": True,
        ...
        "source_count": 0,
        "selected_count": 0,
        "mail_packet_sync": {"ok": True, "skipped": True, "reason": "mail_runtime_memory_is_authoritative"},
        "updated_at": _now_text(),
    }
    ...
    try:
        ...
        from backend.core.fanxiu.history_museum.packet_capture.mail_sync import sync_fanxiu_mail_packets
        from backend.db import engine
...
def sync_fanxiu_capture_paths(
    ...
    *,
    ...
    if scan_existing_decoded:
        decoded_digests, decoded_streams_by_digest, decoded_sources_by_digest = _decoded_capture_index(data_dir)
    else:
        decoded_digests = set()
    ...
    for raw_path in paths:
        ...
        try:
            if path.stat().st_size <= MIN_PCAP_BYTES:
                ...
                continue
            valid_pcap, validation_reason = _validate_sealed_pcap(path)
            if not valid_pcap:
                errors.append({"path": str(path), "error": validation_reason})
                continue
        ...
        if (
            ...
            continue
        try:
            with _capture_decode_lock(digest):
...
def catch_up_fanxiu_packet_facts(
    *,
    ...
    # The just-sealed segment is authoritative.  A segment may also have been
    # rotated immediately before the request, so cover that adjacent boundary
    # too.  Choosing only the larger file can silently prefer an older busy
    # segment over the new one-shot gameplay packet.
    capture_candidates = list(
        dict.fromkeys([pcap_path, *([str(boundary_pcap)] if boundary_pcap else [])])
    )
    ...
    capture_paths = capture_candidates
    ...
    return {
        ...
        "boundary_pcap": str(boundary_pcap or ""),
        "capture_candidates": [
            {"path": candidate, "size": candidate_sizes.get(candidate, 0), "selected": True}
            for candidate in capture_candidates
        ],
...
def sync_fanxiu_live_capture_backlog(
    *,
    ...
    previous_cursor_pcap = str(
        previous_state.get("confirmed_cursor_pcap") or previous_state.get("cursor_pcap") or ""
    ) if isinstance(previous_state, dict) else ""
    # Older workers could append one unprocessed path before enforcing the
    # pass limit. The contiguous-cursor reducer then stopped at that missing
    # state and left the confirmed cursor behind even though the newest
    # scanned capture was already confirmed. Repair that state only when no
    # unresolved gap exists and the latest path has explicit confirmation.
    if use_cursor and isinstance(previous_state, dict) and not previous_state.get("has_unconfirmed_gap") and not previous_errors:
        latest_scanned_path = str(previous_state.get("latest_scanned_pcap") or "")
        latest_scanned_mtime = float(previous_state.get("latest_scanned_mtime") or 0)
        latest_state = next(
            (
                item
                for item in previous_state.get("pcap_states") or []
                if isinstance(item, dict) and str(item.get("path") or "") == latest_scanned_path
            ),
            None,
        )
        if (
            latest_state
            and str(latest_state.get("status") or "") in {"decoded", "already_decoded"}
            and latest_scanned_mtime > previous_cursor_mtime
        ):
            previous_cursor_mtime = latest_scanned_mtime
            previous_cursor_pcap = latest_scanned_path
    ...
    confirmed_cursor_pcap = previous_cursor_pcap
    ...
    decode_attempt_limit = (
        max(1, int(max_decode_attempts))
        if max_decode_attempts is not None
        else decode_limit * 3
    )

    for path in stable_paths:
        if len(decoded) >= decode_limit or decode_attempts >= decode_attempt_limit or scanned >= max_scanned:
            break
        ...
        if not target_stream_ids:
            ...
            continue
        backfill_sources = decoded_sources_by_digest.get(digest) or []
        if not scan_existing_decoded and not backfill_sources:
            backfill_sources = _current_decoded_sources_for_capture(
                path,
                data_dir=data_dir,
                digest=digest,
            )
        ...
        try:
            with _capture_decode_lock(digest):
                result = _decode_runtime_capture_with_timeout(
                    ...
                    data_dir=data_dir,
                    max_streams=max(1, int(max_streams)),
                    timeout_seconds=max(1.0, float(decode_timeout_seconds)),
                )
            actual_decoded_stream_ids = _decoded_stream_ids_from_result(result)
            if (
                not actual_decoded_stream_ids
    ...
    if use_cursor:
        confirmed_cursor_mtime, confirmed_cursor_pcap = _confirmed_cursor_from_contiguous_states(
            ...
            previous_pcap=previous_cursor_pcap,
        )
    payload = {
        "schema_version": PACKET_INSIGHT_WORKER_SCHEMA_VERSION,
        "ok": bool(
...
def sync_fanxiu_capture_maintenance_backlog(
    *,
    ...
    include_mail_business_backlog: bool = False,
    ...
    result = sync_fanxiu_live_capture_backlog(
        ...
        limit=limit,
        scan_existing_decoded=False,
        max_decode_attempts=max(1, int(limit)),
    )
    ...
    if include_mail_business_backlog:
        bounded_mail_sync = sync_fanxiu_mail_business_backlog(
            data_dir=data_dir,
            latest_limit=DEFAULT_MAINTENANCE_MAIL_LATEST_LIMIT,
            historical_limit=DEFAULT_MAINTENANCE_MAIL_HISTORICAL_LIMIT,
        )
    else:
        bounded_mail_sync = {
            "ok": True,
...
class FanxiuPacketInsightWorker:
    def __init__(
        ...
        *,
        ...
        # Foreground decoding and historical maintenance are two independent
        # service lines.  The foreground lock only serializes the periodic
        # realtime scan with an urgent caller-requested catch-up.  Maintenance
        # has its own re-entry guard and must never make a caller wait for a
        # historical replay to finish.
    ...
    def start(self) -> None:
        with self._lock:
            ...
            persisted_realtime = _load_json(_worker_state_path(), {})
            if not isinstance(persisted_realtime, dict):
                persisted_realtime = {}
    ...
    def scan_once(self) -> dict[str, Any]:
        ...
        if not self._foreground_lock.acquire(blocking=False):
            return {
                "ok": True,
                "skipped": True,
                "reason": "foreground_cycle_active",
                "mode": "realtime_scan",
                "updated_at": _now_text(),
            }
        try:
    ...
    def _scan_once_ingestion(self) -> dict[str, Any]:
        ...
        result = sync_fanxiu_live_capture_backlog(
            ...
            limit=2,
            scan_existing_decoded=False,
            decode_timeout_seconds=DEFAULT_REALTIME_DECODE_TIMEOUT_SECONDS,
    ...
    def maintenance_once(self) -> dict[str, Any]:
        ...
        if not self._maintenance_lock.acquire(blocking=False):
            return {
                "ok": True,
                "skipped": True,
                "reason": "maintenance_cycle_active",
                "mode": "maintenance",
                "updated_at": _now_text(),
            }
        try:
    ...
    def _maintenance_once_ingestion(self) -> dict[str, Any]:
        ...
        result = sync_fanxiu_capture_maintenance_backlog(
            ...
            include_mail_business_backlog=False,
    ...
    def catch_up_once(
        ...
        *,
        reason: str = "manual",
        foreground_wait_seconds: float = 30.0,
    ) -> dict[str, Any]:
        self._catch_up_waiting.set()
        # A catch-up is the foreground service guarantee.  It waits only for
        # the current bounded realtime pass, while the waiting flag prevents a
        # new periodic pass from overtaking it.  Historical maintenance uses a
        # different lane and is intentionally irrelevant here.
        acquired = self._foreground_lock.acquire(
            timeout=max(0.0, float(foreground_wait_seconds))
        )
        if not acquired:
            ...
            return {
                ...
                "caught_up": False,
                "reason": "foreground_cycle_timeout",
    ...
    def _maintenance_loop(self) -> None:
        ...
        while not self._maintenance_stop_event.is_set():
            gate = _scheduled_maintenance_resource_gate()
            wait_seconds = self.maintenance_interval_seconds
            if gate.get("defer"):
                with self._lock:
                    self._last_maintenance_result = {
                        "ok": True,
                        "skipped": True,
                        "mode": "maintenance_scheduled",
                        "phase": "deferred_for_host",
                        "updated_at": _now_text(),
                        **gate,
                    }
                wait_seconds = min(
                    self.maintenance_interval_seconds,
                    DEFAULT_MAINTENANCE_PRESSURE_RETRY_SECONDS,
                )
            else:
                try:
                    with self._lock:
                        self._maintenance_cycle_started_at = time.time()
                    result = self.maintenance_once()
                    if isinstance(result, dict):
                        result.setdefault("resource_gate", gate)
                except Exception as exc:
                    with self._lock:
            ...
            self._maintenance_stop_event.wait(wait_seconds)
