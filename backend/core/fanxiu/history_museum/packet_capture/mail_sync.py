...
from backend.core.fanxiu.mail.seat_events import parse_fanxiu_seat_eviction_event
...
def _mail_i18n_template_values(
    ...
    *,
    ...
    for match in re.finditer(r"\$([^$]+)\$", template or ""):
        ...
        elif placeholder in {"NUM", "N"}:
            if raw_num_values:
                raw_num_values.popleft()
            if queues.get("NUM"):
                value = str(queues["NUM"].popleft())
        ...
        for index, frame in enumerate(frames):
            ...
            if name in MAIL_SOURCE_PROTOCOLS:
                ...
                for mail_vo in _iter_mail_vos(parsed):
                    ...
                    seat_event = parse_fanxiu_seat_eviction_event(
                        mail_vo.get("type"), mail_vo, mail_id=mail_id
                    )
                    if seat_event is not None:
                        payload["seat_eviction_event"] = seat_event
    ...
    for record in records:
        ...
        if runtime_action == "missing_from_list" and record_status not in {"claimed", "deleted", "missing_from_list"}:
            ...
            if not _mail_evidence_has_server_action(evidence, record_status):
                record.status = "seen"
                # The UI action is useful routing evidence even when it is not
                # strong enough to prove the final server status.  Reverting
                # the status must not also erase the next-action policy.
                preserve_runtime_policy = True
                ...
                session.add(record)
        policy = runtime_action if preserve_runtime_policy else fanxiu_mail_action_policy_for_record(record)
        if str(record.action_policy or "") == policy:
            continue
        record.action_policy = policy
        session.add(record)
