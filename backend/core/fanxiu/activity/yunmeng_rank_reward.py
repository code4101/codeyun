...
class YunmengRankRewardConfigError(RuntimeError):
...
def _cached_config_rows(
    ...
    try:
        ...
        raise YunmengRankRewardConfigError(f"无法读取凡修配置表 {name}") from exc
    rows = payload if isinstance(payload, list) else payload.get("rows", payload)
    if isinstance(rows, dict):
        rows = list(rows.values())
    if not isinstance(rows, list):
        raise YunmengRankRewardConfigError(f"凡修配置表 {name} 不是有效行列表")
...
def load_activity_rank_reward_tiers(
    *,
    ...
    if activity is None:
        raise YunmengRankRewardConfigError(
...
def load_yunmeng_rank_reward_tiers(**kwargs: Any) -> list[dict[str, Any]]:
    """Compatibility alias for the generic activity rank reward loader."""
