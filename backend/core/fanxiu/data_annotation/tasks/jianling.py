...
class JianlingTaskMixin:
    def _execute_jianling_cuiling_task(
        ...
        for round_index in range(1, max_rounds + 1):
            ...
            numbers, level_text = runtime.ocr_numbers_in_shapes(
                349,
                ("等级",),
                frame_data_url=frame,
            )
            valid_levels = [int(value) for value in numbers if 0 <= int(value) <= 10000]
            level = max(valid_levels) if valid_levels else None
            level_context = "等级" in str(level_text) or "圆满" in str(level_text)
            ...
            if level is not None and level >= target_level and (scene_id == 349 or level_context):
            ...
            runtime.long_press_shape(349, "淬灵", duration=press_seconds)
            yield from runtime.wait_action_settle(0.8)
