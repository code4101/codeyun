...
def _function_name(card: dict[str, Any], index: int) -> str:
    prefix = "guard" if card.get("scope") == "guard" else "action"
...
def _build_compile_prompt(card: dict[str, Any], function_name: str) -> str:
    ...
    return textwrap.dedent(
        ...
        - ctx.yield_tick(reason=""): 守护规则完成一次处理后让出本轮 tick。
...
def _assemble_script(cards: list[dict[str, Any]], compiled_cards: list[dict[str, Any]]) -> str:
    guards = [card for card in compiled_cards if card.get("scope") == "guard"]
    actions = [card for card in compiled_cards if card.get("scope") != "guard"]
    manifest_cards = [
        {
            "id": card.get("id"),
            "scope": card.get("scope") or "action",
    ...
    guard_names = [card["function_name"] for card in guards]
    action_names = [card["function_name"] for card in actions]
    return (
        ...
        + snippets
        + "\n\n"
        + f"GUARD_FUNCTIONS = {[name for name in guard_names]!r}\n"
        + f"ACTION_FUNCTIONS = {[name for name in action_names]!r}\n\n"
        + textwrap.dedent(
            """
            def main() -> None:
                ctx = FanxiuPseudoRuntime(CONTEXT)
                functions = globals()
                guard_cards = [card for card in CARD_MANIFEST if card.get("scope") == "guard"]
                action_cards = [card for card in CARD_MANIFEST if card.get("scope") != "guard"]
                for card in guard_cards:
