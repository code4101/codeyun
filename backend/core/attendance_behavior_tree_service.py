...
def _matches_attendance_script(
    ...
    *,
    ...
    while index < len(args):
        ...
        if text in {"-m", "/m"}:
            if index + 1 < len(args) and args[index + 1] in ATTENDANCE_MODULE_NAMES:
                return "cmd:attendance-module"
            return None
        if text.startswith("-m") and len(text) > 2:
            return "cmd:attendance-module" if text[2:] in ATTENDANCE_MODULE_NAMES else None
        ...
        return "cmd:attendance-script" if _normalize_path(text) == expected_script else None
    return None
