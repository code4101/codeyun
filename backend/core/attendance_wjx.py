...
class WjxSession:
    ...
    def close(self) -> None:
        try:
            ...
            logger.debug("Failed to close WJX tab", exc_info=True)
...
"""
    ...
    for _ in range(max_rounds):
        try:
            ...
            logger.debug("Failed to clear WJX popups", exc_info=True)
            break
        total += current
        if not current:
            break
...
def _cleanup_extra_wjx_tabs(session: WjxSession) -> int:
    ...
    for tab in tabs:
        ...
        try:
            ...
            logger.debug("Failed to close extra WJX tab", exc_info=True)
    return closed
...
def _fill_login_form_if_needed(session: WjxSession, username: str, password: str) -> None:
    ...
    try:
        ...
        logger.debug("No WJX agreement checkbox found", exc_info=True)

    _click_login_submit(tab)


def ensure_logged_in(session: WjxSession, *, username: str, password: str, target_url: str) -> None:
    tab = session.tab
    tab.get(target_url)
    try:
...
def open_edit_page(session: WjxSession, *, activity_id: str | int, question_title: str = DEFAULT_COURSE_QUESTION_TITLE) -> None:
    tab = session.tab
...
def enter_course_question_edit_mode(session: WjxSession, *, question_title: str = DEFAULT_COURSE_QUESTION_TITLE) -> None:
    tab = session.tab
...
def read_course_options(session: WjxSession, *, question_title: str = DEFAULT_COURSE_QUESTION_TITLE) -> dict[str, Any]:
