...
import {
  ...
  importFanxiuModaoInvasionExchangeListFromOcr,
  importFanxiuModaoInvasionPersonalRankingsFromOcr,
  saveFanxiuModaoInvasionExchangeList,
  type FanxiuActivityItem,
  type FanxiuModaoInvasionExchangeItem,
  type FanxiuModaoInvasionOcrImportResponse,
  type FanxiuModaoInvasionPersonalRankingItem,
  type FanxiuModaoInvasionPersonalRankingOcrImportResponse,
  type FanxiuModaoInvasionRecord,
  type FanxiuModaoInvasionSnapshot,
} from '@/api/fanxiu';
import { useUserStore } from '@/store/userStore';
import { useAutoSave } from '@/utils/useAutoSave';

type FanxiuModaoInvasionRecordInput = Partial<FanxiuModaoInvasionRecord> & {
  title?: string;
  remark?: string;
  personal_rankings?: Partial<FanxiuModaoInvasionPersonalRankingItem>[];
  items?: Partial<FanxiuModaoInvasionExchangeItem>[];
};

type FanxiuModaoInvasionSnapshotInput = Partial<FanxiuModaoInvasionSnapshot> & {
  items?: Partial<FanxiuModaoInvasionExchangeItem>[];
  records?: FanxiuModaoInvasionRecordInput[];
  label?: string;
  title?: string;
  remark?: string;
};

const DEFAULT_RECORD_LABEL = '32跨';
const LOCAL_SELECTION_STORAGE_PREFIX = 'fanxiu:modao-invasion-selection';
const MODAO_INVASION_DRAFT_SCHEMA_VERSION = 'v3';
...
type PendingImportSection = 'personal-ranking' | 'exchange';
...
const snapshot = ref<FanxiuModaoInvasionSnapshot>(createEmptySnapshot());
...
const modaoActivities = computed(() => {
  return activityItems.value.filter(item => normalizeText(item.name).includes('魔道入侵'));
});

const orderedRecords = computed(() => snapshot.value.records);

const activityById = computed<Record<string, FanxiuActivityItem>>(() => {
  return Object.fromEntries(modaoActivities.value.map(activity => [activity.id, activity])) as Record<string, FanxiuActivityItem>;
});
...
const unboundActivities = computed(() => {
  ...
  return modaoActivities.value.filter(activity => !boundActivityIds.has(activity.id));
});
...
const autoSave = useAutoSave<FanxiuModaoInvasionSnapshot>({
  debounceMs: 800,
  equals: (left, right) => serializeSnapshot(left) === serializeSnapshot(right),
  storageKey: () => (canEdit.value ? buildDraftStorageKey() : null),
  save: async value => saveFanxiuModaoInvasionExchangeList(prepareSnapshot(value)),
  onError: (error: unknown) => {
    const anyError = error as any;
    ElMessage.error(anyError?.response?.data?.detail || anyError?.message || '自动保存魔道入侵活动数据失败');
  },
});
...
function createEmptySnapshot(): FanxiuModaoInvasionSnapshot {
  return { records: [] };
}

function buildDraftStorageKey() {
  const scope = userStore.user?.id ?? userStore.user?.username ?? 'fanxiu';
  return `fanxiu:modao-invasion:${MODAO_INVASION_DRAFT_SCHEMA_VERSION}:${scope}`;
}
...
function buildRecordId(): string {
  return buildEntityId('modao-invasion-record');
}

function buildRowId(): string {
  return buildEntityId('modao-invasion-item');
}
...
function normalizeItem(
  item: Partial<FanxiuModaoInvasionExchangeItem> | null | undefined,
...
function normalizePersonalRankingItem(
  item: Partial<FanxiuModaoInvasionPersonalRankingItem> | null | undefined,
): FanxiuModaoInvasionPersonalRankingItem {
  return {
    id: normalizeText(item?.id) || buildEntityId('modao-invasion-personal-ranking'),
...
function createEmptyRecord(activityId: string): FanxiuModaoInvasionRecord {
  const activity = activityById.value[activityId];
  return {
    id: buildRecordId(),
    activity_id: activityId,
    label: normalizeText(activity?.name) || '魔道入侵',
    personal_rankings: [],
    items: [],
  };
}
...
function matchRecordToActivity(record: FanxiuModaoInvasionRecordInput | null | undefined, activity: FanxiuActivityItem): boolean {
...
function resolveRecordActivityId(record: FanxiuModaoInvasionRecordInput | null | undefined): string {
  const explicitActivityId = normalizeText(record?.activity_id);
  if (explicitActivityId && activityById.value[explicitActivityId]) {
    return explicitActivityId;
  }

  return modaoActivities.value.find(activity => matchRecordToActivity(record, activity))?.id || '';
}
...
function sortPersonalRankings(items: FanxiuModaoInvasionPersonalRankingItem[]) {
...
function normalizeGenericRecord(
  record: FanxiuModaoInvasionRecordInput | null | undefined,
...
function normalizeSnapshot(
  value: FanxiuModaoInvasionSnapshotInput | null | undefined,
): FanxiuModaoInvasionSnapshot {
  let rawRecords: FanxiuModaoInvasionRecordInput[] = [];
...
function prepareSnapshot(value: FanxiuModaoInvasionSnapshotInput): FanxiuModaoInvasionSnapshot {
...
function serializeSnapshot(value: FanxiuModaoInvasionSnapshot): string {
  return JSON.stringify(prepareSnapshot(value));
}

function createEmptyRow(): FanxiuModaoInvasionExchangeItem {
  return {
    id: buildRowId(),
    name: '',
...
function createEmptyPersonalRankingRow(): FanxiuModaoInvasionPersonalRankingItem {
  return {
    id: buildEntityId('modao-invasion-personal-ranking'),
    rank: 0,
    name: '',
    plane: '',
    merit: 0,
...
function getRecordActivity(record: Pick<FanxiuModaoInvasionRecord, 'activity_id'> | null | undefined) {
  const activityId = normalizeText(record?.activity_id);
  return activityId ? activityById.value[activityId] || null : null;
}

function getRecordTitle(record: Pick<FanxiuModaoInvasionRecord, 'activity_id' | 'label'>) {
...
function getImportSectionLabel(section: PendingImportSection) {
  return section === 'personal-ranking' ? '个人榜' : '兑换宝阁';
}
...
function normalizeRowText(row: FanxiuModaoInvasionExchangeItem) {
  row.name = normalizeText(row.name);
}

function normalizePersonalRankingRow(row: FanxiuModaoInvasionPersonalRankingItem) {
...
function getTotalMagicCrystal(row: FanxiuModaoInvasionExchangeItem) {
...
function addRecord() {
  ...
  if (!activityId) {
    ElMessage.warning('活动列表里没有可新增的魔道入侵活动');
    return;
  }
...
function getActivityOptionsForRecord(record: FanxiuModaoInvasionRecord) {
  return modaoActivities.value.filter(activity => {
...
function handleRecordActivityChange(record: FanxiuModaoInvasionRecord, nextActivityIdRaw: string) {
...
function addRow(record: FanxiuModaoInvasionRecord) {
  record.items.push(createEmptyRow());
}

function addPersonalRankingRow(record: FanxiuModaoInvasionRecord) {
...
function handlePersonalRankingRankChange(record: FanxiuModaoInvasionRecord, row: FanxiuModaoInvasionPersonalRankingItem) {
  normalizePersonalRankingRow(row);
  sortPersonalRankings(record.personal_rankings);
}

function buildMergeKey(item: Pick<FanxiuModaoInvasionExchangeItem, 'name' | 'magic_crystal_cost' | 'purchase_limit'>) {
  return [
...
function applyImportedItems(recordId: string, response: FanxiuModaoInvasionOcrImportResponse) {
  const record = getRecordById(recordId);
  if (!record) {
...
function applyImportedPersonalRankings(recordId: string, response: FanxiuModaoInvasionPersonalRankingOcrImportResponse) {
  const record = getRecordById(recordId);
  if (!record) {
...
async function importImage(target: PendingImportTarget, image: File) {
  ...
  try {
    if (target.section === 'exchange') {
      const response = await importFanxiuModaoInvasionExchangeListFromOcr(image);
    ...
    const response = await importFanxiuModaoInvasionPersonalRankingsFromOcr(image);
...
async function loadSnapshot() {
  ...
  try {
    ...
    snapshot.value = prepareSnapshot((restoredSnapshot ?? normalizedRemoteSnapshot) as FanxiuModaoInvasionSnapshotInput);
    ...
    ElMessage.error(error?.response?.data?.detail || error?.message || '读取魔道入侵活动数据失败');
  } finally {
    loading.value = false;
  }
}
...
<template>
  <div class="modao-invasion-page" v-loading="loading">
    <div class="page-header">
      <h2 class="page-title">活动列表 · 魔道入侵</h2>
    ...
    <el-card shadow="never" class="table-card">
      <div v-if="canEdit && modaoActivities.length" class="page-actions">
        <el-select
          v-model="pendingAddActivityId"
          filterable
          class="page-actions__select"
          placeholder="选择要新增的魔道入侵活动"
          no-data-text="暂无可新增的魔道入侵活动"
        >
          <el-option
          ...
          <div class="activity-meta">
            <div class="field-row">
              ...
              <el-select
                ...
                filterable
                placeholder="选择活动列表中的魔道入侵活动"
                no-data-text="暂无可选的魔道入侵活动"
              ...
              <table class="exchange-table">
                <thead>
                  <tr>
                    ...
                    <th class="col-number">所需魔晶</th>
                    <th class="col-number">限购数量</th>
                    <th class="col-total">总魔晶</th>
                    <th class="col-consume">消耗魔晶</th>
                    <th v-if="canEdit" class="col-actions">操作</th>
                  </tr>
                </thead>
      ...
      <div v-else class="empty-state">
        <div class="empty-state__title">
          {{ modaoActivities.length ? '暂无已添加活动' : '活动列表里暂无魔道入侵活动' }}
        </div>
        <div class="empty-state__description">
          {{
            modaoActivities.length
              ? '从上方选择一个魔道入侵活动后新增，它会作为新的活动文档出现在最上面。'
              : '先到“活动列表”里配置魔道入侵活动，这里才能绑定新增。'
          }}
        </div>
      </div>
    </el-card>
...
<style scoped>
.modao-invasion-page {
  --modao-number-control-width: 24px;
  display: flex;
...
.exchange-table :deep(.number-column .el-input-number.is-controls-right .el-input__wrapper) {
  padding-right: calc(var(--modao-number-control-width) + 6px);
}
...
.exchange-table :deep(.number-column .el-input-number.is-controls-right .el-input-number__increase) {
  width: var(--modao-number-control-width);
  right: 0;
}
