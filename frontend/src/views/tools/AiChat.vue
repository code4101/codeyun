<template>
  <div class="ai-chat-page">
    <section class="hero-panel">
      <div class="hero-copy">
        <div class="eyebrow">AI工具 / AI聊天</div>
        <h1>AI聊天</h1>
      </div>
      ...
      <div class="hero-actions">
        ...
        <el-button
          size="large"
          :icon="Delete"
          :disabled="!messages.length && !attachments.length && !draft.trim()"
        ...
        <section class="panel-card">
          <div class="panel-header panel-header-actions-only">
            <div class="panel-header-actions">
              ...
              <el-button text @click="goToConfigPage">
                去配置
              </el-button>
            </div>
          </div>
              ...
              <el-form-item label="参与模型">
                <div class="selected-model-editor">
                  ...
                  <div v-else class="selected-model-empty">
                    还没有加入模型。先从下方模型池里添加一个或多个。
                  </div>

                  <div class="selected-model-add-row">
                    <el-select
                      v-model="pendingModelOptionId"
                      filterable
                      placeholder="从已配置模型池里添加"
                          ...
                          {{
                            assistantMessage.pending
                              ...
                              : (assistantMessage.error
                                  ? '生成异常'
                                  : (assistantMessage.created_at ? formatTime(assistantMessage.created_at) : '已完成'))
                          }}
                        </span>
                      </div>
                    </button>
              ...
              >
                ...
                <div class="response-detail-body">
                  ...
                  <div v-else class="message-content">
                    {{
                      selectedAssistantMessage.pending
                        ? '正在等待模型回复...'
                        : '该模型当前还没有可展示的正文内容。'
                    }}
                  </div>
...
import { Close, CopyDocument, Delete, Picture, RefreshRight } from '@element-plus/icons-vue'
...
async function clearConversation() {
  if (!messages.value.length && !attachments.value.length && !draft.value.trim()) {
    return
  }

  try {
    await ElMessageBox.confirm('将清空当前聊天记录、待发送图片和输入框内容。', '清空 AI 聊天', {
